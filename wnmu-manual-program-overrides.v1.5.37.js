(function () {
  'use strict';
  const VERSION = 'v1.5.37-true-imported-schedule-overwrite';
  const TABLE = 'wnmu_monthly_schedule_overrides'; // legacy only; v1.5.37 does not write or render override-table records.
  ...
  async function loadTableOverrides() {
    // v1.5.37: manual commits physically update imported month schedule_json.
  ...
  async function deactivateRow(row) { throw new Error('Remove committed program is disabled in v1.5.37 because commits now overwrite imported schedule_json. Restore from backup/reimport if a committed replacement needs to be undone.'); }
  ...
  async function commitFromMenu(menu) {
    ...
    try {
      ...
      menuStatus(menu, `Imported schedule data overwritten. Replaced ${committed.replaced_count || 0} imported block(s); preserved ${committed.preserved_split_count || 0} split piece(s). Reloading from the table…`);
