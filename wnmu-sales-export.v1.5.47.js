(function () {
  'use strict';
  const VERSION = 'v1.5.47-sales-programming-view';
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.47` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.47`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}</div>`;
    }
  }
