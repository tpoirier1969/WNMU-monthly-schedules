...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.13-hybrid-duration-parser';
  const state = { parsed: {} };
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.13 hybrid duration parser', days, weeks };
  ...
  function publishabilityFor(parsed) {
    ...
    if (diagnostics.inferredDurations.length) warnings.push(`${diagnostics.inferredDurations.length} program(s) used inferred duration because no duration token was parsed`);
