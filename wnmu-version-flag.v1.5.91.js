(function () {
  ...
  const PACKAGE_VERSION = 'v1.5.91';
  const PACKAGE_LABEL = 'custom tag stability and menu reload guard';
  const KNOWN_PARTS = [
    { key: 'package', label: 'Delivered package', version: `${PACKAGE_VERSION} ${PACKAGE_LABEL}` },
    { key: 'home', label: 'Home page shell', version: 'index.html v1.5.91' },
    { key: 'schedule-shell', label: 'Schedule page shell', version: 'index131/133 HTML v1.4.1 + package v1.5.91' },
    { key: 'builder-shell', label: 'Month Builder shell', version: 'month-builder HTML v1.4.1 / parser v1.4.17' },
    { key: 'interactions', label: 'Cell interactions / box notes', global: 'WNMU_CELL_INTERACTIONS_VERSION', fallback: 'v1.5.91 custom tag stability and submit guard' },
    ...
    { key: 'manual-overrides', label: 'Imported schedule overwrite menu', global: 'WNMU_MANUAL_PROGRAM_OVERRIDES_VERSION', fallback: 'v1.5.91 commit button single-pass menu plus reload guard' },
    ...
    { key: 'framework', label: 'Fixed framework / scroll area', global: 'WNMU_FIXED_FRAMEWORK_VERSION', fallback: 'v1.5.91 fixed framework no menu reflow' },
  ...
  function pageShellVersion() {
    ...
    if (page === 'index.html' || !page.includes('.html')) return 'Home page shell v1.5.85';
    if (/index13[13]\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.85`;
    if (/month-builder\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.85`;
    return `${page} + package v1.5.85`;
  }
