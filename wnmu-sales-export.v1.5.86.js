(function () {
  'use strict';
  const VERSION = 'v1.5.86-sponsor-guide-grouped-soft-distinct-palette';
  ...
  function canMergeDailyTopicBlocks(current, next) {
    ...
    // Keep literal program-title blocks separate unless they are the same title.
    if (current.titleMode === 'title' || next.titleMode === 'title') return blockKey(current) === blockKey(next);
    // Sponsorship guide rule: on a single day, adjacent programs in the same topic become one topic block.
    return topicBlockKey(current) === topicBlockKey(next);
  }

  function mergeDetailText(existing, incoming) {
    const out = [];
    String(existing || '').split(/\s+•\s+/).forEach(part => {
      const clean = part.trim();
      if (clean && !out.includes(clean)) out.push(clean);
    });
    String(incoming || '').split(/\s+•\s+/).forEach(part => {
      const clean = part.trim();
      if (clean && !out.includes(clean)) out.push(clean);
    });
    return out.join(' • ');
  ...
  function daySegments(day) {
    ...
    for (let slot = START_SLOT; slot < END_SLOT; slot += 1) {
      ...
      if (!current || !canMergeDailyTopicBlocks(current, block)) {
        ...
        current.detail = mergeDetailText(current.detail, block.detail);
      }
    }
    if (current) segments.push(current);
    return segments;
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.86` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.86`;
