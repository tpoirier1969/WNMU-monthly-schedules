(function () {
  'use strict';
  const VERSION = 'v1.5.89-bright-fundraiser-fixed-framework';
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      #wnmuCellMenu .wnmu-custom-tag-row { display:grid!important; grid-template-columns:minmax(0,1fr) auto!important; gap:8px!important; align-items:center!important; margin:6px 0 2px!important; padding:8px!important; border:2px solid rgba(23,52,95,.45)!important; border-radius:10px!important; background:#f8fafc!important; box-shadow:inset 0 0 0 1px rgba(255,255,255,.78)!important; }
      #wnmuCellMenu #wnmuCellCustomTagText { border:2px solid #17345f!important; border-radius:8px!important; background:#fff!important; color:#111!important; min-height:36px!important; box-shadow:inset 0 1px 2px rgba(15,23,42,.14)!important; }
      #wnmuCellMenu #wnmuCellCustomTagText:focus { outline:3px solid rgba(49,95,140,.24)!important; outline-offset:1px!important; }
      ${checkRowCss()}
  ...
  let measureRaf = 0;
  let measureTimer = 0;
  function scheduleMeasureStickyBlock(delay = 0) {
  ...
  function installCellMenuLayoutObserver() {
    ...
    if ('MutationObserver' in window) {
      const obs = new MutationObserver(mutations => {
        const relevant = mutations.some(m => Array.from(m.addedNodes || []).some(n => n && n.nodeType === 1 && (n.id === 'wnmuCellMenu' || n.querySelector?.('#wnmuCellMenu'))));
        if (relevant) window.requestAnimationFrame(run);
      });
  ...
  function installMeasurementHooks() {
    measureStickyBlock(); [50,120,300,900,1800,3500].forEach(ms => window.setTimeout(() => scheduleMeasureStickyBlock(0), ms));
    window.addEventListener('resize', () => scheduleMeasureStickyBlock(180), { passive: true });
    ...
    if (target && 'MutationObserver' in window) {
      ...
      const observer = new MutationObserver(mutations => {
        if (mutations.some(m => Array.from(m.addedNodes || []).some(looksLikeGridContent))) scheduleMeasureStickyBlock(160);
      });
