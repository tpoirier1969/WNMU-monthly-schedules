(function () {
  const VERSION = 'v1.5.82-satellite-paint-resilient';
  ...
  function applySatelliteVisual(cell, isSatellite) {
    if (!cell) return;
    const hasRealTag = cell.classList.contains('marked');

    cell.classList.toggle('wnmu-satellite-feed', !!isSatellite);
    cell.dataset.satelliteFeed = isSatellite ? 'true' : 'false';

    // Default/preselected Satellite Feed cells get a direct background color.
    // If a real planning tag is present, let the real tag color win, but keep
    // the satellite class/data so a later tag cleanup cannot forget this cell.
