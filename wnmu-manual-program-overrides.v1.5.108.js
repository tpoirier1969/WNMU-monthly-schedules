(function () {
  'use strict';
  const VERSION = 'v1.5.108-staged-cache-skip-menu-scrub';
  ...
  const state = { readyKey: '', hydrated: false, tableAvailable: false, loadError: '', menuObserverInstalled: false, committedRows: [], storageGuardInstalled: false, scrubbing: false, lastSharedCleanup: null, lastLocalCleanup: null, storageScrubTimer: 0, stagedRecordsSig: '', stagedRecordsCache: null };
  ...
  function transientMenuWorkActive() { return Number(window.WNMU_MENU_BULK_EDIT_ACTIVE_UNTIL || 0) > Date.now(); }
  function invalidateStagedRecordsCache() { state.stagedRecordsSig = ''; state.stagedRecordsCache = null; }
  ...
  function installStorageGuard() {
    ...
    Storage.prototype.setItem = function wnmuManualProgramSetItemGuard(key, value) {
      ...
      try {
        if (key === storageKey() || key === canonicalStoreKey()) invalidateStagedRecordsCache();
        if (!state.scrubbing && !transientMenuWorkActive() && (key === storageKey() || key === canonicalStoreKey())) {
          ...
          scheduleStorageScrub('localStorage-write', 450);
        }
      } catch (err) {
  ...
  function allStagedRecords() {
    if (!stagedMain || !stagedCanon) loadStagingStores();
    const sig = `${Object.keys(stagedCanon || {}).length}::${Object.keys(stagedMain || {}).length}`;
    if (state.stagedRecordsCache && state.stagedRecordsSig === sig) return state.stagedRecordsCache;
    const out = [];
    const seen = new Set();
    for (const store of [stagedCanon, stagedMain]) {
      Object.entries(store || {}).forEach(([key, raw]) => {
        ...
        if (!rec) return;
        const recSig = `${rec.date}__${rec.time}__${rec.text}`;
        if (seen.has(recSig)) return;
        seen.add(recSig);
        out.push(rec);
      });
    }
    state.stagedRecordsSig = sig;
    state.stagedRecordsCache = out.sort((a, b) => (b.durationMin - a.durationMin) || (a.date + a.time).localeCompare(b.date + b.time));
    return state.stagedRecordsCache;
  }
