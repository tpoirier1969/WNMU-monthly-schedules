(function () {
  'use strict';
  const VERSION = 'v1.5.71-sales-hsb-15-90-palette';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      :root {
        ...
        --sales-kids:#f2efe0; --sales-health:#e0f2e9; --sales-financial:#eee0f2; --sales-news:#e0e8f2;
        --sales-food:#f2e9e0; --sales-home:#f2ede0; --sales-travel:#e0f2f0; --sales-michigan:#e0ecf2; --sales-local:#e0f2e2;
        --sales-arts:#e6e0f2; --sales-environmental:#e6f2e0; --sales-science:#e0eff2; --sales-history:#f2eae0; --sales-documentary:#e3e0f2;
        --sales-drama:#f2e0ee; --sales-talk:#e0e9f2; --sales-native:#f2e7e0; --sales-variety:#eff2e0; --sales-choice:#e5f2e0; --sales-specials:#e0e3f2; --sales-fundraiser:#f2e0e6; --sales-program:#f4f6f8; --sales-blank:#ffffff;
      }
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.71` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.71`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.71 note: sales category colors use a softer pastel target: about 7.5% saturation and 95% brightness; fundraiser detection still uses only exact date/time placements from the actual Fundraising Schedule.</small></div>`;
    }
  }
