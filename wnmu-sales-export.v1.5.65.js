(function () {
  'use strict';
  const VERSION = 'v1.5.65-sales-children-title-color-titleclean';
  ...
  function collectPledgeScheduleProgramKeys(value, depth = 0) {
    ...
    if (looksLikeScheduledProgram) {
      ...
      // v1.5.64: intentionally do not collect code/program_id values here.
    }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      :root {
        ...
        --sales-kids:#fff3b8; --sales-health:#e5f4ee; --sales-financial:#f1ddff; --sales-news:#e4edf8;
        --sales-food:#ffe1c2; --sales-home:#f2ead6; --sales-travel:#d8eef8; --sales-michigan:#dbeeff; --sales-local:#e2f5df;
      ...
      * { box-sizing:border-box; }
      ...
      .sales-detail { margin-top:3px; font-size:10.5px; color:#5f6878; opacity:.72; line-height:1.18; }
      .sales-program .sales-label { color:#667184; }
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.63` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.63`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.64 note: title cleanup uses shared Supabase corrections when available; fundraiser detection uses exact title matches from actual Pledge Schedule data only.</small></div>`;
    }
  }
