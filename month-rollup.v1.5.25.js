(function () {
  'use strict';
  const VERSION = 'v1.5.25-standalone-month-rollup';
  ...
  function buildRows(entries, marks, manualOverrides) {
    ...
    const manualStarts = new Set((manualOverrides || []).map(row => `${row.date}__${row.slot_time}`));
    entries.forEach(entry => {
      if (manualStarts.has(`${entry.date}__${entry.time}`)) return;
  ...
  function render(row, rows, sourceInfo) {
    ...
    $('scheduleLink').href = `${CONFIG.page}?month=${encodeURIComponent(month)}&v=1.5.25`;
    $('homeLink').href = 'index.html?v=1.5.25';
