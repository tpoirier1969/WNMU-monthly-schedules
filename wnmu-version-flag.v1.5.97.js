(function () {
  ...
  const PACKAGE_VERSION = 'v1.5.97';
  const PACKAGE_LABEL = 'episode-code guarded import plus persistent auto-tags';
  const KNOWN_PARTS = [
    { key: 'package', label: 'Delivered package', version: `${PACKAGE_VERSION} ${PACKAGE_LABEL}` },
    { key: 'schedule-shell', label: 'Schedule page shell', version: 'index131/133 HTML v1.4.1 + package v1.5.97' },
    { key: 'builder-shell', label: 'Month Builder shell', version: 'month-builder HTML v1.4.1 / parser v1.4.19' },
    { key: 'interactions', label: 'Cell interactions / box notes', global: 'WNMU_CELL_INTERACTIONS_VERSION', fallback: 'v1.5.93 menu stability and full-grid repaint reduction' },
    { key: 'remote-storage', label: 'Remote localStorage sync', global: 'WNMU_REMOTE_STORAGE_VERSION', fallback: 'v1.5.85 Supabase-backed monthly localStorage' },
    { key: 'manual-programs', label: 'Manual schedule overwrite', global: 'WNMU_MANUAL_PROGRAMS_VERSION', fallback: 'v1.5.64 manual schedule overwrite' },
    { key: 'manual-overrides', label: 'Imported schedule overwrite menu', global: 'WNMU_MANUAL_PROGRAM_OVERRIDES_VERSION', fallback: 'v1.5.93 debounced commit cleanup and reload guard' },
    ...
    { key: 'title-review', label: 'Title cleanup review', global: 'WNMU_TITLE_REVIEW_VERSION', fallback: 'v1.5.65 quiet title review' },
    { key: 'auto-tags', label: 'Auto tag helper', global: 'WNMU_AUTO_TAG_HELPERS_VERSION', fallback: 'v1.5.96 persistent New Series/New Season auto-tags' },
    { key: 'sticky', label: 'Sticky week/day headers', global: 'WNMU_STICKY_WEEK_HEADERS_VERSION', fallback: 'v1.5.64 sticky panel/week/table header block' },
    { key: 'framework', label: 'Fixed framework / scroll area', global: 'WNMU_FIXED_FRAMEWORK_VERSION', fallback: 'v1.5.93 fixed framework no menu reflow' },
  ...
  const LABELS = [
    ...
    [/^wnmu-remote-storage/i, 'Remote localStorage sync'],
    ...
    [/^wnmu-manual-programs/i, 'Manual schedule overwrite'],
    [/^wnmu-manual-program-overrides/i, 'Imported schedule overwrite'],
  ...
  function cleanFlag(text) {
    return String(text || '')
      ...
      .replace(/\s*•\s*satellite paint fix[^•]*/ig, '')
  ...
  function pageShellVersion() {
    ...
    if (page === 'index.html' || !page.includes('.html')) return 'Home page shell v1.5.96';
    if (/index13[13]\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.96`;
    if (/month-builder\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.96`;
    return `${page} + package v1.5.96`;
  }
  ...
  function componentRows() {
    ...
    if (window.WNMU_REMOTE_STORAGE_VERSION) add('Remote localStorage sync', window.WNMU_REMOTE_STORAGE_VERSION, 'global');
    if (window.WNMU_MANUAL_PROGRAMS_VERSION) add('Manual schedule overwrite', window.WNMU_MANUAL_PROGRAMS_VERSION, 'global');
    ...
    if (window.WNMU_AUTO_TAG_HELPERS_VERSION) add('Auto tag helper', window.WNMU_AUTO_TAG_HELPERS_VERSION, 'global');
  ...
  function setMainFlags() {
    ...
    if (flag) {
      const base = cleanFlag(flag.textContent) || 'renderer status';
      flag.textContent = `${compact} • ${base}`;
    }
  ...
  function normalizeUrlVersion() {
    try {
      ...
      const wanted = PACKAGE_VERSION.replace(/^v/i, '');
      if (url.searchParams.get('v') !== wanted) {
        url.searchParams.set('v', wanted);
        window.history.replaceState(window.history.state, document.title, url.toString());
      }
    } catch (err) {
      console.warn('WNMU version URL sync skipped.', err);
    }
  }
