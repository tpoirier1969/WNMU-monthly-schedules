(function () {
  'use strict';
  const VERSION = 'v1.5.68-sales-runtime-helper-fix';
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.68` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.68`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.68 note: restored the Sales View Supabase helper functions; fundraiser detection still uses only exact date/time placements from the actual Fundraising Schedule.</small></div>`;
    }
  }
