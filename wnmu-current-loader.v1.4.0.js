...
(function () {
  ...
  async function boot() {
    try {
      const res = await fetch(`${CONFIG.registryFile}?v=${encodeURIComponent(CONFIG.loaderVersion || CONFIG.buildVersion || 'v1')}`, { cache: 'no-store' });
      if (!res.ok) throw new Error(`Could not load ${CONFIG.registryFile} (${res.status})`);
