(function () {
  const VERSION = 'v1.4.24-satellite-initial-paint-fix';
  ...
  function injectStyles() {
    if (document.getElementById('wnmuSatelliteFeedV1424Styles')) return;
    const style = document.createElement('style');
    style.id = 'wnmuSatelliteFeedV1424Styles';
    style.textContent = `
  ...
  function applySatelliteVisual(cell, isSatellite) {
    ...
    // This is the v1.4.24 bug fix:
    // default/preselected Satellite Feed cells get a direct background color.
    // As soon as a real planning tag is present, remove it so real tag color wins.
  ...
  function paintEntry(entryId, root) {
    ...
    scope.querySelectorAll(`.program-cell[data-entry-id="${cssEscape(entryId)}"]`).forEach(cell => {
      applySatelliteVisual(cell, effectiveSatellite(entryId));
    });
  }
  ...
  function schedulePaint(root) {
    requestAnimationFrame(() => paintRoot(root || document));
  }
  ...
  function installDelayedSweeps() {
    ...
    [150, 500, 1200, 2500, 5000].forEach(ms => {
      window.setTimeout(() => paintRoot(document.getElementById('weekGrids') || document), ms);
    });
  }
  ...
  function start() {
    ...
    if (flag && !flag.textContent.includes('satellite paint fix')) {
      flag.textContent = `${flag.textContent} • satellite paint fix 1.4.24`;
    }
  }
