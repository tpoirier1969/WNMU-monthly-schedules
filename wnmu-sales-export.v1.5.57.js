(function () {
  'use strict';
  const VERSION = 'v1.5.57-title-review-corrections';
  ...
  let pledgeScheduleTitleSet = new Set();
  let pledgeScheduleCodeSet = new Set(); // retained only for legacy diagnostics; not used for Sales View classification
  ...
  function collectPledgeScheduleProgramKeys(value, depth = 0) {
    if (!value || depth > 10) return;
    if (Array.isArray(value)) {
      value.forEach(item => collectPledgeScheduleProgramKeys(item, depth + 1));
      return;
    }
    if (typeof value !== 'object') return;

    const titleKeys = [
      'program_title', 'programTitle', 'title', 'program_name', 'programName',
      'selectedProgramTitle', 'display_title', 'displayTitle', 'name'
    ];
    // Do NOT collect program codes from the pledge schedule. Codes/IDs are too easy
    // to over-match against monthly schedule JSON and can turn the whole sales grid
    // into Fundraiser/Pledge. Fundraiser classification is now title-exact only
    // against actual Pledge Schedule data.

    const looksLikeScheduledProgram = Object.keys(value).some(key =>
      /program|placement|airing|slot|title/i.test(key)
    );

    if (looksLikeScheduledProgram) {
      for (const key of titleKeys) {
        const raw = value[key];
        if (typeof raw === 'string') {
          const title = cleanProgramTitle(raw);
          const titleKey = norm(title);
          // Avoid adding generic labels or schedule names as program titles.
          if (titleKey && titleKey.length > 3 && !/pledge|fundraiser|schedule|drive|week|day/.test(titleKey)) {
            pledgeScheduleTitleSet.add(titleKey);
          }
        }
      }
      // v1.5.57: intentionally do not collect code/program_id values here.
    }

    Object.keys(value).forEach(key => collectPledgeScheduleProgramKeys(value[key], depth + 1));
  }

  async function loadOptionalPledgeLookup() {
    pledgeScheduleTitleSet = new Set();
    pledgeScheduleCodeSet = new Set();

    // Sales View should only treat programs as pledge/fundraising when their
    // normalized title is present in the actual Pledge Schedule data. Do not use
    // pledge library/catalog rows, and do not use schedule codes/IDs for matching.
    const rows = await optionalRest('/rest/v1/pledge_fundraiser_schedules?select=*&limit=2000');
    if (Array.isArray(rows)) rows.forEach(row => collectPledgeScheduleProgramKeys(row));

    window.WNMU_SALES_PLEDGE_LOOKUP = {
      source: 'pledge_fundraiser_schedules only',
      titleCount: pledgeScheduleTitleSet.size,
      codeCount: 0,
      titleSamples: Array.from(pledgeScheduleTitleSet).slice(0, 12),
  ...
  function isPledgeProgram(entry) {
    ...
    // Exact normalized title match only. No code, NOLA, ID, or JSON substring
    // matching here; those caused broad false positives in v1.5.53/v1.5.54.
    return !!(titleKey && pledgeScheduleTitleSet.has(titleKey));
  }
  ...
  function buildTitleAuditPanel() {
    const corrections = Array.from(titleAuditState.known.values()).sort((a, b) => b.count - a.count || a.from.localeCompare(b.from));
    const variants = titleAuditState.variants || [];
    const userList = userCorrectionsList();
    let html = '<section class="sales-title-audit"><h3>Title cleanup check</h3>';
    html += '<p>Display-only review. Save a correction here if the Sales View title is wrong; it will be used next time this sheet is rebuilt in this browser.</p>';
    html += '<div class="sales-title-correction-form"><label>Imported/wrong title <input id="salesTitleWrong" type="text" placeholder="Great Get Aways"></label><label>Correct display title <input id="salesTitleCorrect" type="text" placeholder="Great Getaways"></label><button type="button" id="salesTitleSaveCorrection">Save correction</button></div>';
    if (userList.length) {
      html += '<div><strong>Your saved display corrections:</strong><ul>';
      userList.slice(0, 80).forEach(item => { html += `<li><code>${esc(item.fromKey)}</code> → <strong>${esc(item.to)}</strong></li>`; });
      html += '</ul></div>';
    }
    if (corrections.length) {
      html += '<div><strong>Display corrections applied:</strong><ul>';
      corrections.slice(0, 40).forEach(item => {
        html += `<li>${esc(item.from)} → <strong>${esc(item.to)}</strong> <span>(${esc(item.source)}, ${item.count}×)</span></li>`;
      });
      html += '</ul></div>';
    }
    if (variants.length) {
      html += '<div><strong>Possible title variants found this month:</strong><ul>';
      variants.slice(0, 40).forEach(group => {
        const bits = group.variants.map(v => `${esc(v.title)} (${v.count}×)`).join(' / ');
        html += `<li>Preferred guess: <strong>${esc(group.preferred)}</strong> — ${bits}</li>`;
      });
      html += '</ul></div>';
    }
    if (!corrections.length && !variants.length && !userList.length) html += '<p>No title variants or display corrections were found.</p>';
    html += '<p>This report is hidden when printing. Corrections do not rewrite schedule data.</p></section>';
    return html;
  }

  function bindTitleAuditControls() {
    const btn = document.getElementById('salesTitleSaveCorrection');
    if (!btn) return;
    btn.addEventListener('click', () => {
      const from = document.getElementById('salesTitleWrong')?.value || '';
      const to = document.getElementById('salesTitleCorrect')?.value || '';
      if (!saveUserCorrection(from, to)) return;
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.57` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.57`;
    ...
    let html = `
      ...
      <p class="sales-note">Sales-facing view: broad blocks are simplified, sponsor-useful titles are shown as secondary detail, and title cleanup is display-only.</p>
      ${buildLegend()}`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.57 note: title cleanup is display-only; fundraiser detection uses exact title matches from actual Pledge Schedule data only.</small></div>`;
    }
  }
