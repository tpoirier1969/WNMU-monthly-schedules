(function () {
  ...
  function monthHref(page, monthKey) {
    const base = page || window.location.pathname.split('/').pop() || 'index131.v1.4.1.html';
    const sep = base.includes('?') ? '&' : '?';
    return `${base}${sep}month=${encodeURIComponent(monthKey)}&v=1.4.6`;
  }
  function samePageMonthHref(monthKey) {
    return monthHref(window.location.pathname.split('/').pop() || '', monthKey);
  }
  function getCfg() {
  ...
  function renderMonthNavFromRows(rows, currentByChannel, channelCode, selectedMonth) {
    ...
    host.innerHTML = months.map(row => {
      ...
      return `<a href="${samePageMonthHref(row.month_key)}" style="color:${isSelected ? '#fff' : '#cfd7ff'};text-decoration:${isSelected ? 'underline' : 'none'}">${esc(row.label || monthLabel(row.month_key))}</a>${suffix}`;
    }).join(' &nbsp;·&nbsp; ');
  }
  function renderAltChannelLinkFromRows(rows, currentByChannel, selectedMonth) {
    ...
    link.href = otherMonth ? monthHref(LIVE_PAGES[otherCode], otherMonth) : `${LIVE_PAGES[otherCode]}?v=1.4.6`;
  ...
  function renderMonthNavFromRegistry(registry, channelCode, selectedMonth) {
    ...
    host.innerHTML = months.map(([monthKey, meta]) => {
      ...
      if (isCurrent) bits.push('current');
      const suffix = bits.length ? ` <span style="opacity:.7">(${bits.join(', ')})</span>` : '';
      return `<a href="${samePageMonthHref(monthKey)}" style="color:${isSelected ? '#fff' : '#cfd7ff'};text-decoration:${isSelected ? 'underline' : 'none'}">${esc(meta.label || monthLabel(monthKey))}</a>${suffix}`;
    }).join(' &nbsp;·&nbsp; ');
  }
  ...
  function stateEntryLooksEmpty(value) {
    ...
    // Explicit false values are meaningful: they suppress automatic/default tags
    // such as Programmer's Choice. Treat any boolean tag state as a saved mark
    // so Supabase mirrors de-selections instead of resurrecting defaults later.
  ...
  async function boot() {
    ...
    try {
      ...
      console.warn('Shared marks preload failed; continuing schedule render.', err);
    }

    if (monthLoaded) loadRenderer();

    try {
      installSharedMarksSync();
      installLegacyNotesPanel();
    } catch (err) {
      console.warn('Shared marks helpers failed after schedule render.', err);
    }
  }
