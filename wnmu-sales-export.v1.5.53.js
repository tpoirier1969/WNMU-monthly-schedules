(function () {
  'use strict';
  const VERSION = 'v1.5.53-sales-view-runtime-build-fix';
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.53` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.53`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.53 note: Sales View restores the missing runtime helpers used by the rule engine, including title keyword matching and schedule slot lookup. Optional pledge lookup failures are skipped without stopping the export.</small></div>`;
    }
  }
