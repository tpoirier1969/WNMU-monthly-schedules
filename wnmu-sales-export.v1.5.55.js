(function () {
  'use strict';
  const VERSION = 'v1.5.55-sales-view-pledge-title-exact-only';
  ...
  function collectPledgeScheduleProgramKeys(value, depth = 0) {
    ...
    if (looksLikeScheduledProgram) {
      ...
      // v1.5.55: intentionally do not collect code/program_id values here.
    }
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.55` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.55`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.55 note: Fundraiser/Pledge detection uses exact title matches from actual Pledge Schedule data only. Codes/IDs/$ detection are intentionally ignored to prevent false positives.</small></div>`;
    }
  }
