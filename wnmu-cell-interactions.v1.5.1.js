(function () {
  ...
  function targetFromCell(cell) {
    ...
    if (cell.dataset.entryId) { entryId = cell.dataset.entryId; dateTime = parseEntryId(entryId); sourceType = 'program'; }
  ...
  function writeCanonicalRecord(target, note) {
    ...
    store[target.key] = { channelCode: channelCode(), monthKey: monthKey() || target.date.slice(0, 7), date: target.date, time: target.time, sourceType: target.sourceType, sourceEntryId: target.entryId || '', text: note.text || '', durationMin: nearestDuration(note.durationMin || 30), tags: note.tags || {}, cleared: !!note.cleared, updatedAt: new Date().toISOString() };
  ...
  function legacyProgramNote(target) {
    ...
    for (const entryId of ids) {
      ...
      return { text: rect.text || '', durationMin: nearestDuration(extra.durationMin || rect.durationMin || Math.max(1, Number(target.cell?.rowSpan || 1)) * 30), tags: item.tags || {}, legacy: 'program', legacyEntryId: entryId };
    }
    return null;
  }
  function legacyBlankNote(target) {
    ...
    for (const key of oldBlankStoreKeys()) {
      ...
      if (mark && typeof mark === 'object' && mark.rectNote && mark.rectNote.text) return { text: mark.rectNote.text || '', durationMin: nearestDuration(mark.rectNote.durationMin || 30), tags: mark.tags || {}, legacy: 'blank' };
    }
    return null;
  }
  ...
  function readTagsFromMenu(menu) { const tags = {}; tagOrder().forEach(tag => { const input = menu?.querySelector(`input[name="${css(tag)}"]`); if (input?.checked) tags[tag] = true; }); return tags; }
  ...
  function cellMenuHtml(target, note) {
    ...
    const tagRows = tagOrder().map(tag => `<label class="check-row ${esc(tagClassName(tag))}"><input type="checkbox" name="${esc(tag)}" ${tags[tag] ? 'checked' : ''}> <span>${esc(tagMeta(tag).label || tag)}</span></label>`).join('');
  ...
  function openCellMenu(target, event) {
    ...
    menu.querySelector('#wnmuCellCloseBtn')?.addEventListener('click', event => { event.preventDefault(); hideCellMenu(); });
    menu.querySelector('#wnmuCellSaveBtn')?.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); saveFromCellMenu(); });
    menu.querySelector('#wnmuCellClearBtn')?.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); clearFromCellMenu(); });
  ...
  function saveFromCellMenu() {
    ...
    const status = menu?.querySelector('#wnmuCellStatus'); if (status) status.textContent = 'Box note saved.';
    return true;
  }
  ...
  function restoreVisibleNotes() {
    ...
    oldBlankStoreKeys().forEach(storeKey => { const store = readJson(storeKey, {}); Object.entries(store).forEach(([oldKey, mark]) => { if (!mark?.rectNote?.text) return; const m = String(oldKey).match(/^(13\.1|13\.3)__(\d{4}-\d{2}-\d{2})__(\d{2}:\d{2})__blank-slot$/); if (!m || m[1] !== channelCode()) return; const target = targetForDateTime(m[2], m[3]); if (!target || readCanonicalRecord(target.key)) return; paintOverlay(target, { text: mark.rectNote.text, durationMin: mark.rectNote.durationMin || 30, tags: mark.tags || {} }); }); });
    const main = readJson(storageKey(), {}); Object.entries(main).forEach(([entryId, item]) => { if (!item?.rectNote) return; const dt = parseEntryId(entryId); if (!dt) return; const target = targetForDateTime(dt.date, dt.time); if (!target || readCanonicalRecord(target.key)) return; const note = legacyProgramNote(target); if (note) paintOverlay(target, note); });
    if (restoreAttempts < 8) { window.clearTimeout(restoreTimer); restoreTimer = window.setTimeout(restoreVisibleNotes, restoreAttempts < 4 ? 350 : 900); }
  }

  function injectStyles() {
    if (document.getElementById('wnmuCellInteractionsV151Styles')) return;
    const style = document.createElement('style'); style.id = 'wnmuCellInteractionsV151Styles';
    style.textContent = `#wnmuCellOverrideLayer{position:absolute;left:0;top:0;width:0;height:0;z-index:2147481000;pointer-events:none}.wnmu-cell-override-box{position:absolute;box-sizing:border-box;background:rgba(255,255,255,.96);border:2px solid rgba(12,18,32,.86);color:#111;border-radius:4px;box-shadow:0 2px 7px rgba(0,0,0,.22);padding:5px 6px 16px;overflow:hidden;font:12px/1.22 system-ui,sans-serif;pointer-events:none}.wnmu-cell-override-text{white-space:normal;overflow:hidden;font-weight:700}.wnmu-cell-override-tags{display:flex;flex-wrap:wrap;gap:3px;margin-bottom:3px}.wnmu-cell-override-tags span{display:inline-flex;align-items:center;border:1px solid rgba(0,0,0,.25);border-radius:999px;padding:1px 5px;font-size:9px;font-weight:800;background:rgba(255,255,255,.72)}.wnmu-cell-override-duration{position:absolute;right:4px;bottom:2px;font-size:9px;opacity:.62}.program-cell.wnmu-has-cell-override>.draw-rect-note,.program-cell.wnmu-has-cell-override>.wnmu-blank-slot-content{display:none!important}#contextMenu{z-index:1}#wnmuCellMenu.wnmu-cell-menu{position:fixed;z-index:2147483600!important;min-width:360px;max-width:460px}.wnmu-cell-note-tools{border:1px solid rgba(255,255,255,.22);border-radius:10px;padding:10px;margin:10px 0;background:rgba(255,255,255,.06)}.wnmu-cell-note-tools legend{font-weight:800}.wnmu-cell-duration-options{display:flex;flex-wrap:wrap;gap:5px;margin:5px 0 8px}.wnmu-cell-pill{display:inline-flex;align-items:center;gap:3px;font-size:12px;border:1px solid rgba(255,255,255,.24);border-radius:999px;padding:4px 7px;cursor:pointer}.wnmu-cell-pill input{margin:0}.manual-rect-text{width:100%;box-sizing:border-box;border-radius:8px;border:1px solid rgba(255,255,255,.28);padding:7px;resize:vertical;font:13px/1.3 system-ui,sans-serif;color:#111;background:#fff}.manual-rect-label{font-size:12px;font-weight:800;margin:6px 0 5px}.manual-rect-help{margin-top:6px;font-size:11px;opacity:.75;line-height:1.25}@media print{#wnmuCellOverrideLayer{display:none!important}}`;
    document.head.appendChild(style);
  }
  function installHandlers() {
    document.addEventListener('contextmenu', event => { const cell = event.target.closest?.('.program-cell'); if (!cell || cell.classList.contains('outside')) return; const target = targetFromCell(cell); if (!target) return; event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); openCellMenu(target, event); }, true);
  ...
  function start() { injectStyles(); installHandlers(); ensureOverlayLayer(); window.WNMU_CELL_INTERACTIONS_VERSION = VERSION; window.setTimeout(restoreVisibleNotes, 600); }
