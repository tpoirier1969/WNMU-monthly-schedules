(function () {
  const VERSION = 'v1.5.83-satellite-feed-isolated-painter';
  ...
  function hasVisibleNonSatellitePlanningTag(cell) {
    if (!cell) return false;
    const pills = Array.from(cell.querySelectorAll?.('.program-tags .tag-pill, .wnmu-cell-override-tags [data-wnmu-overlay-tag]') || []);
    return pills.some(pill => {
      const tag = String(pill.getAttribute('data-wnmu-overlay-tag') || '').trim();
      const label = normalizedFeedText(pill.textContent || '');
      if (tag && tag !== SATELLITE_TAG) return true;
      if (!label) return false;
      if (label === 'satellite feed' || label === 'pbs feed' || label === 'pbs feeds') return false;
      return true;
    });
  }
  ...
  function applySatelliteVisual(cell, isSatellite) {
    ...
    const hasRealTag = hasVisibleNonSatellitePlanningTag(cell);
    ...
    if (isSatellite && hasRealTag) {
      // Keep the satellite identity for the checkbox, but let a real planning
      // tag/background win visually.
      if (cell.dataset.satelliteFeedManaged === 'true') cell.style.backgroundColor = '';
      cell.dataset.satelliteFeedManaged = 'true';
      cell.style.setProperty('--satellite-feed-background', SAT_COLOR);
      return;
    }

    if (!isSatellite && wasManagedSatellite) {
      cell.style.backgroundColor = '';
      cell.dataset.satelliteFeedManaged = 'false';
  ...
  function schedulePaint(root) {
    ...
    pendingPaintRoot = pendingPaintRoot === document ? document : target;
    if (paintRaf) return;
    paintRaf = requestAnimationFrame(() => {
      const nextRoot = pendingPaintRoot || document;
      paintRaf = 0;
  ...
  function scheduleResizePaint() {
    ...
    resizePaintTimer = window.setTimeout(() => {
      schedulePaint(document.getElementById('weekGrids') || document);
    }, 220);
  }

  function repaintSoon(delay = 0) {
    window.setTimeout(() => schedulePaint(document.getElementById('weekGrids') || document), delay);
  }

  function installHooks() {
    window.WNMU_REPAINT_SATELLITE_FEEDS = () => schedulePaint(document.getElementById('weekGrids') || document);
  ...
  function installDelayedSweeps() {
    ...
    [150, 500, 1200, 2500, 5000, 9000, 15000].forEach(ms => {
      window.setTimeout(() => schedulePaint(document.getElementById('weekGrids') || document), ms);
    });
  }
