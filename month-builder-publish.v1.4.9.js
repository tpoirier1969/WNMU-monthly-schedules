...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.9-duration-hole-safe-parser';
  const state = { parsed: {} };
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    for (let offset = 0; offset < totalDays; offset += 1) {
      ...
      const normalizedEntries = deduped.map((entry, idx) => {
        ...
        return { date: dateKey, day: dayName, dayName, time: entry.time, title: entry.title, episode: entry.episode, seasonStart: inferSeasonStart(entry.episode), endTime, durationMin: slotCount * 30, slotCount, sourceStartTime: entry.sourceStartTime || '', sourceDurationMin: entry.sourceDurationMin || null, sourceEndTime: entry.sourceEndTime || '', durationSource: explicitLooksUsable ? 'explicit-pdf-timepair' : 'next-program-inference' };
      });
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.9 duration/hole-safe parser', days, weeks };
  ...
  function publishabilityFor(parsed) {
    ...
    else if (fullDays < expected) warnings.push(`${expected - fullDays} day(s) contain visible schedule holes`);
    ...
    return { ok: fatal.length === 0, fatal, warnings, fullDays, expected };
  }
  ...
  function renderParsedSummary(channelCode, parsed) {
    ...
    const holeLines = (parsed.verification.dailyCoverage || [])
      .filter(day => (day.missingSlots || []).length || (day.overlapSlots || []).length)
      .slice(0, 12)
      .map(day => `${day.date}: missing=${(day.missingSlots || []).length}, overlaps=${(day.overlapSlots || []).length}`)
      .join('\n');
