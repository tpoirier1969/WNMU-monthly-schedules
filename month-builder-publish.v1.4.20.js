...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.20-strict-day-cell-parser';
  const state = { parsed: {} };
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.20 strict seven-day-cell parser with episode/title guards', days, weeks };
  ...
  function publishabilityFor(parsed) {
    ...
    if (durationStats.explicit < Math.max(1, Math.floor((parsed.entryCount || 0) * 0.35))) {
      fatal.push(`only ${durationStats.explicit} duration/start pairs matched; parser reliability is too low`);
    }
    ...
    if (hardParseIssues.length) {
      fatal.push(`${hardParseIssues.length} parser alignment issue(s) need review before publishing`);
    }
