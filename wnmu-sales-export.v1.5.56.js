(function () {
  'use strict';
  const VERSION = 'v1.5.56-sales-view-title-cleanup-audit';
  ...
  function cleanProgramTitle(value) {
    ...
    if (letters.length >= 4 && letters === letters.toUpperCase()) {
      const fixed = smartTitleCase(title);
      rememberTitleCorrection(title, fixed, 'case cleanup');
      title = fixed;
    }
    return title;
  }
  ...
  function collectPledgeScheduleProgramKeys(value, depth = 0) {
    ...
    if (looksLikeScheduledProgram) {
      ...
      // v1.5.56: intentionally do not collect code/program_id values here.
    }
  ...
  function buildTitleAuditPanel() {
    ...
    if (!corrections.length && !variants.length) return '';
    ...
    if (variants.length) {
      ...
      variants.slice(0, 40).forEach(group => {
        ...
        html += `<li>Preferred: <strong>${esc(group.preferred)}</strong> — ${bits}</li>`;
      });
      html += '</ul></div>';
    }
    html += '<p>This report is hidden when printing. Corrections are display-only and do not rewrite schedule data.</p></section>';
    return html;
  }
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.56` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.56`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.56 note: title cleanup is display-only; fundraiser detection uses exact title matches from actual Pledge Schedule data only.</small></div>`;
    }
  }
