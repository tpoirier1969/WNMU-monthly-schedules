...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.22-nonstrict-import-recovery';
  const state = { parsed: {} };
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.22 non-strict recovery parser with episode-code title guard', days, weeks };
  ...
  function collectCoverageDiagnostics(schedule, verification) {
    ...
    return { exceptions, inferredDurations, suspectTitles };
  }
