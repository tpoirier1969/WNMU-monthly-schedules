(function () {
  'use strict';
  const VERSION = 'v1.5.40-true-imported-overwrite-preserved-scroll-refresh';
  const TABLE = 'wnmu_monthly_schedule_overrides'; // legacy only; v1.5.40 does not write or render override-table records.
  ...
  async function loadTableOverrides() {
    // v1.5.40: manual commits physically update imported month schedule_json.
  ...
  async function deactivateRow(row) { throw new Error('Remove committed program is disabled in v1.5.40 because commits now overwrite imported schedule_json. Restore from backup/reimport if a committed replacement needs to be undone.'); }
