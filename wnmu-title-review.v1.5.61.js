(function () {
  'use strict';
  const VERSION = 'v1.5.61-title-review-supabase-corrections';
  ...
  function bindInside(root) {
    ...
    root.querySelectorAll('[data-delete-title-key]').forEach(btn => btn.addEventListener('click', async () => {
      const key = btn.getAttribute('data-delete-title-key') || '';
    ...
    root.querySelectorAll('.title-review-save-inline').forEach(btn => btn.addEventListener('click', async () => {
      ...
      if (saved.localOk) {
        markRowSaved(editor, from, to); // this removes/closes the inline editor
