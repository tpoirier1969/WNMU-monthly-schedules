(function () {
  ...
  const VERSION = 'v1.5.77-custom-planning-label';
  ...
  function tagDisplayLabel(tag, otherLabel = '') {
    if (tag === OTHER_KEY) return cleanOtherLabel(otherLabel) || 'Custom';
  ...
  function paintOverlay(target, note) {
    ...
    const noteOtherLabel = cleanOtherLabel(note.otherLabel || note.planningOtherLabel || '');
    const tags = Object.keys(note.tags || {}).filter(tag => note.tags[tag] && (tag !== OTHER_KEY || noteOtherLabel));
    const pills = tags.length ? `<div class="wnmu-cell-override-tags">${tags.map(tag => `<span>${esc(tagDisplayLabel(tag, noteOtherLabel))}</span>`).join('')}</div>` : '';
  ...
  function openCellMenu(target, event) {
    ...
    if (customInput) {
      customInput.addEventListener('input', () => updateOtherMenuLabel(menu, customInput.value));
    }
