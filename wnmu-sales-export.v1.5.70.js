(function () {
  'use strict';
  const VERSION = 'v1.5.70-sales-hsb-15-90-palette';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      :root {
        ...
        --sales-kids:#e6dfc3; --sales-health:#c3e6d4; --sales-financial:#ddc3e6; --sales-news:#c3d2e6;
        --sales-food:#e6d3c3; --sales-home:#e6dac3; --sales-travel:#c3e6e2; --sales-michigan:#c3d9e6; --sales-local:#c3e6c5;
        --sales-arts:#d1c3e6; --sales-environmental:#cce6c3; --sales-science:#c3e1e6; --sales-history:#e6d6c3; --sales-documentary:#c7c3e6;
        --sales-drama:#e6c3e0; --sales-talk:#c3d4e6; --sales-native:#e6d1c3; --sales-variety:#e0e6c3; --sales-choice:#d3e6c3; --sales-specials:#c3c9e6; --sales-fundraiser:#e6c3cc; --sales-program:#eef1f5; --sales-blank:#ffffff;
      }
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.70` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.70`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.70 note: sales category colors use an HSB/HSV target of about 15% saturation and 90% brightness; fundraiser detection still uses only exact date/time placements from the actual Fundraising Schedule.</small></div>`;
    }
  }
