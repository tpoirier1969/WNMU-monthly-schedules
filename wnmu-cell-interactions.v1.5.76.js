(function () {
  ...
  const VERSION = 'v1.5.76-visible-custom-other-labels';
  ...
  function tagDisplayLabel(tag, otherLabel = '') {
    if (tag === OTHER_KEY) return cleanOtherLabel(otherLabel) || 'Other';
    return tagMeta(tag).label || tag;
  }
  function otherRowLabel(otherLabel = '') { return cleanOtherLabel(otherLabel) ? `Other: ${cleanOtherLabel(otherLabel)}` : 'Other…'; }
  function updateOtherMenuLabel(menu, otherLabel = '') {
    const input = menu?.querySelector(`input[name="${css(OTHER_KEY)}"]`);
    const span = input?.closest?.('.check-row')?.querySelector('span');
    if (span) span.textContent = otherRowLabel(otherLabel);
    if (menu) menu.dataset.wnmuOtherLabel = cleanOtherLabel(otherLabel);
  }
  function promptOtherLabel(current = '') {
    const value = window.prompt('Type the custom Planning Tag label for Other:', cleanOtherLabel(current));
    if (value === null) return null;
    return cleanOtherLabel(value);
  }
  ...
  function canonicalFromBase(target) {
    ...
    return { text: base.text || base.rectNote?.text || '', durationMin: base.durationMin || base.rectNote?.durationMin || 30, tags: base.tags || {}, otherLabel: cleanOtherLabel(base.otherLabel || base.planningOtherLabel || ''), canonical: true, baseCanonical: true };
  }
  function writeCanonicalRecord(target, note) {
    const duration = nearestDuration(note.durationMin || 30);
    const cleanTags = normalizeTagsForStorage(note.tags || {});
    const otherLabel = cleanTags[OTHER_KEY] ? cleanOtherLabel(note.otherLabel || note.planningOtherLabel || '') : '';
    const record = { channelCode: channelCode(), monthKey: monthKey() || target.date.slice(0, 7), date: target.date, time: target.time, sourceType: target.sourceType, sourceEntryId: target.entryId || '', text: note.text || '', durationMin: duration, tags: cleanTags, otherLabel, cleared: !!note.cleared, updatedAt: new Date().toISOString() };
    ...
    if (mainKey) {
      ...
      if (target.entryId) {
        ...
        main[target.entryId] = { ...existing, tags: cleanTags, otherLabel, planningOtherLabel: otherLabel, rectNote: record.cleared ? null : { x: 4, y: 4, w: 92, h: 92, text: record.text, durationMin: duration, anchor: 'left', cellOverrideV15: true } };
      }
  ...
  function legacyProgramNote(target) {
    ...
    for (const entryId of ids) {
      ...
      return { text: rect.text || item.text || '', durationMin: nearestDuration(extra.durationMin || rect.durationMin || Math.max(1, Number(target.cell?.rowSpan || 1)) * 30), tags: item.tags || {}, otherLabel: cleanOtherLabel(item.otherLabel || item.planningOtherLabel || ''), legacy: 'program', legacyEntryId: entryId };
    }
    return null;
  }
  function legacyBlankNote(target) {
    ...
    for (const key of oldBlankStoreKeys()) {
      ...
      if (mark && typeof mark === 'object' && mark.rectNote && mark.rectNote.text) return { text: mark.rectNote.text || '', durationMin: nearestDuration(mark.rectNote.durationMin || 30), tags: mark.tags || {}, otherLabel: cleanOtherLabel(mark.otherLabel || mark.planningOtherLabel || ''), satelliteFeed: mark.satelliteFeed, legacy: 'blank' };
    }
    return null;
  }
  function readEffectiveNote(target) {
    ...
    if (canon) return canon.cleared ? null : { text: canon.text || '', durationMin: nearestDuration(canon.durationMin || 30), tags: canon.tags || {}, otherLabel: cleanOtherLabel(canon.otherLabel || canon.planningOtherLabel || ''), planningOtherLabel: cleanOtherLabel(canon.otherLabel || canon.planningOtherLabel || ''), canonical: true };
  ...
  function explicitFalseTags() { const tags = {}; allMenuTags().forEach(tag => { tags[tag] = false; }); return tags; }
  function normalizeTagsForStorage(tags) { const out = {}; allMenuTags().forEach(tag => { out[tag] = !!(tags && tags[tag]); }); return out; }
  ...
  function dominantTag(tags) { const active = activeTagKeys(tags, false); const priority = Array.isArray(cfg().tagPriority) ? cfg().tagPriority : tagOrderBase(); return priority.find(tag => active.includes(tag)) || active[0] || ''; }
  function tagPillHtml(tag, otherLabel = '') { return `<span class="tag-pill" style="--tag-color:${esc(tagMeta(tag).color || '#ddd')}">${esc(tagDisplayLabel(tag, otherLabel))}</span>`; }
  function applyCellTagVisual(target, tags, otherLabel = '') {
    const cell = target?.cell;
    if (!cell) return;
    const cleanTags = normalizeTagsForStorage(tags || {});
    const active = activeTagKeys(cleanTags, false);
    const isSat = !!cleanTags[SATELLITE_KEY];
    cell.classList.toggle('marked', active.length > 0);
    if (active.length) {
      const dom = dominantTag(cleanTags);
    ...
    if (content) {
      ...
      if (active.length) {
        ...
        wrap.innerHTML = active.slice(0, 6).map(tag => tagPillHtml(tag, otherLabel)).join('');
  ...
  function paintOverlay(target, note) {
    ...
    const pills = tags.length ? `<div class="wnmu-cell-override-tags">${tags.map(tag => `<span>${esc(tagDisplayLabel(tag, note.otherLabel || ''))}</span>`).join('')}</div>` : '';
  ...
  function cellMenuHtml(target, note) {
    ...
    const otherLabel = cleanOtherLabel(note?.otherLabel || note?.planningOtherLabel || '');
    const tagRows = allMenuTags().map(tag => `<label class="check-row ${esc(tagClassName(tag))}"><input type="checkbox" name="${esc(tag)}" ${tags[tag] ? 'checked' : ''}> <span>${esc(tag === OTHER_KEY ? otherRowLabel(otherLabel) : (tagMeta(tag).label || tag))}</span></label>`).join('');
  ...
  function positionMenu(menu, x, y) {
    ...
    menu.style.left = `${Math.max(12, Math.min(x, window.innerWidth - r.width - 12))}px`;
    menu.style.top = `${Math.max(12, Math.min(y, window.innerHeight - r.height - 12))}px`;
  }
  ...
  function openCellMenu(target, event) {
    ...
    updateOtherMenuLabel(menu, note?.otherLabel || note?.planningOtherLabel || '');
    const otherInput = menu.querySelector(`input[name="${css(OTHER_KEY)}"]`);
    if (otherInput) {
      otherInput.addEventListener('change', () => {
        if (!otherInput.checked) { updateOtherMenuLabel(menu, ''); return; }
        const label = promptOtherLabel(menu.dataset.wnmuOtherLabel || note?.otherLabel || note?.planningOtherLabel || '');
        if (!label) { otherInput.checked = false; updateOtherMenuLabel(menu, ''); return; }
        updateOtherMenuLabel(menu, label);
      });
    }
  ...
  function saveFromCellMenu() {
    ...
    let otherLabel = cleanOtherLabel(menu?.dataset?.wnmuOtherLabel || '');
    if (tags[OTHER_KEY] && !otherLabel) {
      const label = promptOtherLabel('');
      if (label) {
        otherLabel = label;
        updateOtherMenuLabel(menu, otherLabel);
      } else {
        tags[OTHER_KEY] = false;
        const otherInput = menu?.querySelector(`input[name="${css(OTHER_KEY)}"]`);
        if (otherInput) otherInput.checked = false;
        updateOtherMenuLabel(menu, '');
      }
    }
    try {
