(function () {
  'use strict';
  const VERSION = 'v1.5.76-performance-safe-visible-other-and-season-start';
  ...
  function tagLabel(tag, otherLabel = ''){ if (tag === OTHER_KEY) return cleanOtherLabel(otherLabel) || 'Other'; return tagMeta()[tag]?.label || (tag === SATELLITE_KEY ? 'Satellite Feed' : tag); }
  ...
  function populateMenuEffectiveTags(){
    ...
    if (menu) {
      menu.dataset.wnmuOtherLabel = otherLabel;
      const otherInput = menu.querySelector(`input[name="${css(OTHER_KEY)}"]`);
      const span = otherInput?.closest?.('.check-row')?.querySelector('span');
      if (span) span.textContent = otherLabel ? `Other: ${otherLabel}` : 'Other…';
    }
  ...
  function writeExplicitTagsForMenu(menu, tags){
    ...
    const writeRecord = (recordKey) => {
      ...
      const otherLabel = clean[OTHER_KEY] ? cleanOtherLabel(menu?.dataset?.wnmuOtherLabel || existing.otherLabel || existing.planningOtherLabel || '') : '';
