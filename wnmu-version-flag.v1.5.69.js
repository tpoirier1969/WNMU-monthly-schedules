(function () {
  ...
  const PACKAGE_VERSION = 'v1.5.69';
  const PACKAGE_LABEL = 'sales pastel tune + prime-time topic fixes';
  const KNOWN_PARTS = [
    { key: 'package', label: 'Delivered package', version: `${PACKAGE_VERSION} ${PACKAGE_LABEL}` },
    { key: 'home', label: 'Home page shell', version: 'index.html v1.5.69' },
    { key: 'schedule-shell', label: 'Schedule page shell', version: 'index131/133 HTML v1.4.1 + package v1.5.69' },
    ...
    { key: 'manual-programs', label: 'Manual schedule overwrite', global: 'WNMU_MANUAL_PROGRAMS_VERSION', fallback: 'v1.5.69 sales pastel tune + prime-time topic fixes' },
    ...
    { key: 'framework', label: 'Fixed framework / scroll area', global: 'WNMU_FIXED_FRAMEWORK_VERSION', fallback: 'v1.5.69 sales pastel tune + prime-time topic fixes' },
  ...
  function pageShellVersion() {
    ...
    if (page === 'index.html' || !page.includes('.html')) return 'Home page shell v1.5.69';
    if (/index13[13]\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.69`;
    if (/month-builder\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.69`;
    return `${page} + package v1.5.69`;
  }
