...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.18-positioned-column-import';
  const state = { parsed: {} };
  ...
  function looksLikeTitleContinuation(line) {
    const cleaned = cleanupCellText(line);
    if (!cleaned || isSeparatorLine(cleaned) || isTimeRowLine(cleaned) || isDurationLine(cleaned) || isEpisodeOnlyLine(cleaned)) return false;
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.18 positioned-column parser', days, weeks };
