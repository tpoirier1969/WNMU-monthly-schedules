(function () {
  ...
  const VERSION = 'v1.4.42-event-driven-blank-slot-menu-targeted-save';
  ...
  function resolveActiveTarget() {
    if (active?.cell && active.date && active.time) return active;
    const menu = document.getElementById('blankSlotContextMenu');
    const date = menu?.dataset?.blankDate || '';
    const time = menu?.dataset?.blankTime || '';
    if (!date || !time) return null;
    const cell = findCellFor(date, time);
    if (!cell) return null;
  ...
  function durationHeightForCell(cell, durationMin) {
    ...
    return Math.max(22, Math.round(baseHeight * slots - 4));
  }

  function ensureLayer() {
    let layer = document.getElementById('wnmuBlankSlotLayerV142');
    if (layer) return layer;
    layer = document.createElement('div');
    layer.id = 'wnmuBlankSlotLayerV142';
    layer.setAttribute('aria-hidden', 'true');
    document.body.appendChild(layer);
    return layer;
  }
  function removeOverlay(key) {
    document.querySelectorAll(`.wnmu-blank-v142-box[data-blank-slot-key="${cssEscape(key)}"]`).forEach(el => el.remove());
  }
  ...
  function renderOverlay(cell, mark) {
    ...
    const overlayHeight = durationHeightForCell(cell, duration);
    const box = document.createElement('div');
    const labels = tagLabels(mark.tags);
    box.className = 'wnmu-blank-v142-box';
    ...
    box.innerHTML = `${labels.length ? `<div class="wnmu-blank-v142-tags">${labels.map(l => `<span>${escapeHtml(l)}</span>`).join('')}</div>` : ''}<div class="wnmu-blank-v142-text">${escapeHtml(mark.rectNote.text)}</div><div class="wnmu-blank-v142-duration">${duration} min</div>`;
  ...
  function ensureMenu() {
    ...
    ['pointerdown','mousedown','mouseup','click'].forEach(type => {
      menu.addEventListener(type, e => { e.stopPropagation(); }, true);
    });
    return menu;
  }
  function hideMenu() {
  ...
  function saveFromMenu(showBoxStatus = true) {
    ...
    if (!target?.cell || !target.date || !target.time) { setStatus('No blank slot selected. Right-click the blank cell again.', 'error'); return false; }
    const mark = currentMenuState();
    const payload = saveMark(target.date, target.time, mark);
    const reread = markFor(target.date, target.time);
    renderCell(target.cell, reread);
    if (showBoxStatus) setStatus(mark.rectNote?.text ? 'Box note saved.' : 'Blank-slot settings saved.', 'ok');
    window.WNMU_LAST_BLANK_SLOT_SAVE = { ...target, mark: payload, savedAt: new Date().toISOString() };
    return true;
  }
  function clearNoteFromMenu() {
    const target = resolveActiveTarget();
    if (!target?.cell || !target.date || !target.time) { setStatus('No blank slot selected. Right-click the blank cell again.', 'error'); return; }
    const mark = currentMenuState();
    mark.rectNote = null;
    const textarea = document.getElementById('blankRectText');
    if (textarea) textarea.value = '';
    saveMark(target.date, target.time, mark);
    renderCell(target.cell, markFor(target.date, target.time));
  ...
  function openMenu(cell, event) {
    ...
    menu.dataset.blankDate = located.date;
    menu.dataset.blankTime = located.time;
    menu.dataset.blankKey = located.key;
    buildMenu(menu, mark);
  ...
  function injectStyles() {
    if (document.getElementById('wnmuBlankSlotMenuV142Styles')) return;
    const style = document.createElement('style');
    style.id = 'wnmuBlankSlotMenuV142Styles';
    style.textContent = `
      ...
      #wnmuBlankSlotLayerV142{position:absolute;left:0;top:0;width:0;height:0;pointer-events:none;z-index:2147483000}.wnmu-blank-v142-box{position:absolute;box-sizing:border-box;background:rgba(255,255,255,.96);color:#111;border:2px solid rgba(10,20,40,.78);border-radius:4px;box-shadow:0 2px 7px rgba(0,0,0,.22);overflow:hidden;padding:4px 5px 14px;font:12px/1.2 system-ui,sans-serif;pointer-events:none}.wnmu-blank-v142-tags{display:flex;gap:3px;flex-wrap:wrap;margin-bottom:3px}.wnmu-blank-v142-tags span{font-size:8px;line-height:1;background:#eee;border:1px solid #bbb;border-radius:999px;padding:2px 4px}.wnmu-blank-v142-text{white-space:normal;overflow:hidden}.wnmu-blank-v142-duration{position:absolute;right:4px;bottom:2px;font-size:9px;opacity:.58}
      @media print{#wnmuBlankSlotLayerV142{display:none!important}.blank-slot-menu{display:none!important}}
    `;
  ...
  function setVersionHint() {
    ...
    if (flag) flag.textContent = `${flag.textContent} • blank slot menu 1.4.42`;
  }
  function start() {
    injectStyles();
    installHandlers();
