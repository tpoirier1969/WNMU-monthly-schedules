...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.26-day-anchor-episode-row-guard';
  const state = { parsed: {} };
  ...
  function lineToPipe(group, cols) {
    ...
    group.items.forEach(item => {
      ...
      const col = dayColumnForX(item.x, cols);
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.26 day-anchored non-strict recovery parser with episode retention and service-code guard', days, weeks };
