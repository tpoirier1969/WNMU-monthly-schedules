(function () {
  'use strict';
  const VERSION = 'v1.5.82-visible-custom-planning-label-satellite-safe';
  ...
  function allTagKeys(){ const list = Array.isArray(cfg().tagOrder) ? cfg().tagOrder.slice() : []; if (!list.includes('newSeason')) list.splice(Math.min(1, list.length), 0, 'newSeason'); if (!list.includes(OTHER_KEY)) list.splice(Math.min(5, list.length), 0, OTHER_KEY); if (!list.includes(SATELLITE_KEY)) list.push(SATELLITE_KEY); return Array.from(new Set(list)); }
