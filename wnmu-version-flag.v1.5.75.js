(function () {
  ...
  const PACKAGE_VERSION = 'v1.5.75';
  const PACKAGE_LABEL = 'custom Other planning tag';
  const KNOWN_PARTS = [
    { key: 'package', label: 'Delivered package', version: `${PACKAGE_VERSION} ${PACKAGE_LABEL}` },
    { key: 'home', label: 'Home page shell', version: 'index.html v1.5.75' },
    { key: 'schedule-shell', label: 'Schedule page shell', version: 'index131/133 HTML v1.4.1 + package v1.5.75' },
    { key: 'builder-shell', label: 'Month Builder shell', version: 'month-builder HTML v1.4.1 / parser v1.4.16' },
    { key: 'interactions', label: 'Cell interactions / box notes', global: 'WNMU_CELL_INTERACTIONS_VERSION', fallback: 'v1.5.75 custom Other planning tag' },
    ...
    { key: 'framework', label: 'Fixed framework / scroll area', global: 'WNMU_FIXED_FRAMEWORK_VERSION', fallback: 'v1.5.75 custom Other planning tag layout' },
  ...
  function pageShellVersion() {
    ...
    if (page === 'index.html' || !page.includes('.html')) return 'Home page shell v1.5.75';
    if (/index13[13]\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.75`;
    if (/month-builder\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.75`;
    return `${page} + package v1.5.75`;
  }
