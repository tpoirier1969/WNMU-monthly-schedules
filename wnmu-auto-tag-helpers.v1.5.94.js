(function () {
  'use strict';
  const VERSION = 'v1.5.94-idle-sanitize-entry-fast-path';
  ...
  function runWhenIdle(fn, timeout = 900){
    ...
    return window.requestAnimationFrame(() => fn({ timeRemaining: () => 8 }));
  }
  ...
  let sanitizeIdleHandle = 0;
  function scheduleSanitize(delay){
    window.clearTimeout(sanitizeTimer);
    cancelIdle(sanitizeIdleHandle);
    sanitizeIdleHandle = 0;
    const wait = Math.max(120, Number(delay ?? 160) || 160);
    sanitizeTimer = window.setTimeout(() => {
      sanitizeIdleHandle = runWhenIdle(() => { sanitizeIdleHandle = 0; sanitizeVisibleHighlights(); }, 1200);
    }, wait);
  }
  ...
  function sanitizeVisibleHighlights(){
    ...
    sanitizeActive = true;
    const cells = Array.from(document.querySelectorAll('td.program-cell[data-entry-id]'));
    let index = 0;
    const finish = () => {
      window.dispatchEvent(new CustomEvent('wnmu:auto-tags-rendered', { detail: { version: VERSION, at: new Date().toISOString(), cells: cells.length } }));
      window.setTimeout(() => window.WNMU_REPAINT_SATELLITE_FEEDS?.(), 40);
      window.setTimeout(() => { sanitizeActive = false; }, 80);
    };
    const processChunk = (deadline) => {
      let processed = 0;
      while (index < cells.length) {
        renderTags(cells[index], effectiveTagsForCell(cells[index]));
        index += 1;
        processed += 1;
        if (processed >= 24 && deadline && typeof deadline.timeRemaining === 'function' && deadline.timeRemaining() < 4) break;
        if (processed >= 48) break;
      }
      if (index < cells.length) runWhenIdle(processChunk, 900);
      else finish();
    };
    runWhenIdle(processChunk, 900);
  }

  function cellForMenu(menu){
    const entryId = menu?.dataset?.wnmuEntryId || '';
    if (entryId) {
      const direct = document.querySelector(`td.program-cell[data-entry-id="${css(entryId)}"]`);
  ...
  function installObservers(){
    ...
    [700,2200].forEach(ms => window.setTimeout(() => { scheduleSanitize(0); scheduleCustomLabelRepair(80); }, ms));
  }
