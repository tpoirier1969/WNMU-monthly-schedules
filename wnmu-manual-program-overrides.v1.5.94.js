(function () {
  'use strict';
  const VERSION = 'v1.5.94-staged-cache-lighter-startup';
  const TABLE = 'wnmu_monthly_schedule_overrides'; // legacy only; v1.5.64 does not write or render override-table records.
  ...
  const state = { readyKey: '', hydrated: false, tableAvailable: false, loadError: '', menuObserverInstalled: false, committedRows: [], storageGuardInstalled: false, scrubbing: false, lastSharedCleanup: null, lastLocalCleanup: null, storageScrubTimer: 0 };
  ...
  async function loadTableOverrides() {
    // v1.5.64: manual commits physically update imported month schedule_json.
  ...
  async function deactivateRow(row) { throw new Error('Remove committed program is disabled in v1.5.64 because commits now overwrite imported schedule_json. Restore from backup/reimport if a committed replacement needs to be undone.'); }
  ...
  let stagedMain = null; let stagedCanon = null; let stagedMeta = null; let stagedOldStores = null; let stagedRecordsCache = null; let stagedRecordsCacheKey = '';
  function invalidateStagedRecordsCache(){ stagedRecordsCache = null; stagedRecordsCacheKey = ''; }
  function loadStagingStores() {
    invalidateStagedRecordsCache();
  ...
  function flushStagingChanges() {
    ...
    stagedMain = stagedCanon = stagedMeta = null; stagedOldStores = null; invalidateStagedRecordsCache();
  }
  ...
  function scheduleCommittedScrub(reason) {
    [120, 900].forEach(ms => window.setTimeout(() => scrubCommittedStaging(reason, true), ms));
  }

  function scheduleStorageScrub(reason, delay = 450) {
    window.clearTimeout(state.storageScrubTimer);
    state.storageScrubTimer = window.setTimeout(() => scrubCommittedStaging(reason || 'localStorage-write', true), Math.max(0, Number(delay) || 0));
  }
  ...
  function installStorageGuard() {
    ...
    Storage.prototype.setItem = function wnmuManualProgramSetItemGuard(key, value) {
      ...
      try {
        if (key === storageKey() || key === canonicalStoreKey()) {
          invalidateStagedRecordsCache();
          if (!state.scrubbing) {
            // Debounce storage-triggered scrub work. Tag/menu edits can write several localStorage
            // records in quick succession; running a full committed-staging scrub after each write
            // makes the page feel slower the longer it is used.
            scheduleStorageScrub('localStorage-write', 900);
          }
        }
      } catch (err) {
  ...
  function allStagedRecords() {
    if (!stagedMain || !stagedCanon) loadStagingStores();
    const cacheKey = `${storageKey()}::${canonicalStoreKey()}::${Object.keys(stagedCanon || {}).length}::${Object.keys(stagedMain || {}).length}`;
    if (stagedRecordsCache && stagedRecordsCacheKey === cacheKey) return stagedRecordsCache;
    const out = [];
    const seen = new Set();
    ...
    stagedRecordsCache = out.sort((a, b) => (b.durationMin - a.durationMin) || (a.date + a.time).localeCompare(b.date + b.time));
    stagedRecordsCacheKey = cacheKey;
    return stagedRecordsCache;
  }
  ...
  function bindCommitButtons(menu) {
    if (!menu) return;
    const commitBtn = menu.querySelector('#wnmuCommitProgramBtn');
    if (commitBtn && commitBtn.dataset.wnmuCommitBound !== '1') {
      commitBtn.dataset.wnmuCommitBound = '1';
    ...
    const removeBtn = menu.querySelector('#wnmuRemoveCommittedProgramBtn');
    if (removeBtn && removeBtn.dataset.wnmuRemoveBound !== '1') {
      removeBtn.dataset.wnmuRemoveBound = '1';
  ...
  function ensureCommitButton(menu) {
    ...
    let actions = menu.querySelector('.wnmu-unified-actions') || menu.querySelector('.wnmu-commit-actions');
    if (!menu.querySelector('#wnmuCommitProgramBtn')) {
      actions = document.createElement('div');
      actions.className = 'rect-actions wnmu-unified-actions wnmu-commit-actions';
    ...
    } else if (actions && !actions.classList.contains('wnmu-unified-actions')) {
      actions.classList.add('wnmu-unified-actions');
    }
    bindCommitButtons(menu);
    prefillCommitted(menu);
  }

  function scheduleMenuEnsure(delay = 0) {
    const run = () => {
      const menu = document.getElementById('wnmuCellMenu');
      if (menu) ensureCommitButton(menu);
    };
  ...
  function installMenuObserver() {
    ...
    // v1.5.93: the right-click menu includes the Commit button in its initial HTML.
    // Bind and prefill on the explicit menu-open event instead of running a delayed burst.
    window.addEventListener('wnmu:cell-menu-opened', () => scheduleMenuEnsure(0));

    document.addEventListener('contextmenu', event => {
      if (event.target?.closest?.('.program-cell, .screen-week-grid td, .wnmu-cell-override-box')) {
        scheduleMenuEnsure(0);
      }
    }, true);
  ...
  function start() { ensureCommitMenuStyles(); installStorageGuard(); installMenuObserver(); tryHydrate(); [500, 1800, 5000].forEach(ms => window.setTimeout(() => { tryHydrate(); cleanupLoadedRowsLocally('startup-local-cleanup'); }, ms)); [2500, 9000].forEach(ms => window.setTimeout(() => scrubCommittedStaging('startup-scrub', true), ms)); window.addEventListener('wnmu:manual-program-overrides-refresh', () => { state.hydrated = false; tryHydrate(); }); }
