...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.21-import-gate-hotfix';
  const state = { parsed: {} };
  ...
  function cleanupCellText(text) {
    ...
    s = s.replace(/(#\d{1,6})\s+([A-Z])\b/g, '$1$2');
    return s;
  }
  const ANCILLARY_CODES = new Set(['APX', 'DVS', 'STE', 'SAP', 'CC', 'HD', 'HDTV', 'VDS', 'VOD']);
  const ANCILLARY_CODE_RE = /\b(?:APX|DVS|STE|SAP|CC|HD|HDTV|VDS|VOD)\b/g;
  ...
  function fixedDayParts(line) {
    const fixed = fixedPipeCells(line);
    if (fixed) {
      const out = fixed.slice(1, 8);
      while (out.length < 7) out.push('');
      return out.slice(0, 7);
    }
    const out = coarseParts(line, true);
    while (out.length < 7) out.push('');
    return out.slice(0, 7);
  }
  function rowTimeFromLine(line) {
    const fixed = fixedPipeCells(line);
    const raw = fixed ? fixed[0] : coarseParts(line, false)[0];
    const match = String(raw || '').match(/\b\d{2}:\d{2}\b/);
    return match ? match[0] : cleanupCellText(raw || '');
  }
  function firstEpisodeToken(primaryText, fallbackText) {
    const primary = episodeTokens(primaryText);
    if (primary.length) return primary[0];
    const fallback = episodeTokens(fallbackText);
    return fallback.length ? fallback[0] : '';
  }
  function allEpisodeTokensForCell(primaryText, fallbackText) {
    const seen = new Set();
    const out = [];
    episodeTokens(`${primaryText || ''} ${fallbackText || ''}`).forEach(token => {
  ...
  function durationPairForCell(durationText, rowTime) {
    const pairs = durationStartPairs(durationText);
    if (!pairs.length) return null;
    const exact = pairs.find(pair => pair.startTime === rowTime);
    return exact || pairs[0] || null;
  }
  function parseRowTriplet(titleLine, episodeLine, durationLine) {
    const rowTime = rowTimeFromLine(titleLine);
    const titleParts = fixedDayParts(titleLine);
    const episodeParts = fixedDayParts(episodeLine);
    const durationParts = fixedDayParts(durationLine);
    const cells = [];

    // v1.4.20: one PDF row always maps to exactly seven day cells.
    // Earlier versions expanded a cell when the episode row contained multiple # tokens.
    // That caused Friday/Saturday episodes to slide into the wrong day columns and let
    // fragments like "#1412H STE #1413H STE" become bogus titles. Do not expand columns.
    for (let i = 0; i < 7; i += 1) {
      const titleRaw = cleanupCellText(titleParts[i] || '');
      const episodeRaw = cleanupCellText(episodeParts[i] || '');
      const durationRaw = cleanupCellText(durationParts[i] || '');
      const title = sanitizeTitleCandidate(titleRaw);
      const episode = firstEpisodeToken(episodeRaw, titleRaw);
      const allEpisodes = allEpisodeTokensForCell(episodeRaw, titleRaw);
      const pair = durationPairForCell(durationRaw, rowTime);
      const issues = [];
      if (!title && episode) issues.push('episode-without-title');
      if (allEpisodes.length > 1) issues.push(`multiple-episode-tokens:${allEpisodes.join(',')}`);
      if (titleRaw && !title && hasRealTitleSignal(titleRaw)) issues.push('title-sanitized-away');
      cells.push({
        title: title || '',
        episode: episode || '',
        ...
        sourceRawTitle: titleRaw,
        sourceRawEpisode: episodeRaw,
        parseIssue: issues.join('; ')
      });
    }
      ...
      if (/^\|\s*\d{2}:\d{2}\s*\|/.test(lines[i])) {
        ...
        parsed.cells.forEach((cell, idx) => {
          if (cell.title || cell.episode) {
            entries.push({ date: weekDates[idx], time: parsed.rowTime, title: cell.title, episode: cell.episode, sourceStartTime: cell.sourceStartTime || '', sourceEndTime: cell.sourceEndTime || '', sourceDurationMin: cell.sourceDurationMin || null, sourceDurationToken: cell.sourceDurationToken || '', sourceStartToken: cell.sourceStartToken || '', sourceRawTitle: cell.sourceRawTitle || '', sourceRawEpisode: cell.sourceRawEpisode || '', parseIssue: cell.parseIssue || '' });
          }
        });
        i = nextIndex - 1;
      }
  ...
  function inferColumnBoundaries(groups) {
    ...
    if (bars.length >= 8) {
      const minDayX = bars[0];
      const maxX = bars[bars.length - 1];
      const dayWidth = Math.max(1, (maxX - minDayX) / 7);
      return { minTimeX, minDayX, maxX, dayWidth, source: 'bar-grid' };
    }
    ...
    const dayWidth = Math.max(1, (maxX - minDayX) / 7);
    return { minTimeX, minDayX, maxX, dayWidth, source: 'text-range' };
  }
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    for (let offset = 0; offset < totalDays; offset += 1) {
      ...
      const normalizedEntries = deduped.map((entry, idx) => {
        ...
        return { date: dateKey, day: dayName, dayName, time: entry.time, title: entry.title, episode: entry.episode, seasonStart: inferSeasonStart(entry.episode), endTime, durationMin: slotCount * 30, slotCount, sourceStartTime: entry.sourceStartTime || '', sourceStartToken: entry.sourceStartToken || '', sourceDurationMin: entry.sourceDurationMin || null, sourceDurationToken: entry.sourceDurationToken || '', sourceRawTitle: entry.sourceRawTitle || '', sourceRawEpisode: entry.sourceRawEpisode || '', parseIssue: entry.parseIssue || '', durationSource: explicitLooksUsable ? 'explicit-pdf-duration-start-pair' : 'inferred-to-next-program', inferredDuration };
      });
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.21 seven-day-cell parser with episode/title guards and warning-only alignment diagnostics', days, weeks };
  ...
  function collectCoverageDiagnostics(schedule, verification) {
    ...
    (schedule.days || []).forEach(day => {
      (day.entries || []).forEach(entry => {
        if (entry.parseIssue) {
          parseIssues.push({ date: day.date, time: entry.time, title: entry.title || '', episode: entry.episode || '', issue: entry.parseIssue, rawTitle: entry.sourceRawTitle || '', rawEpisode: entry.sourceRawEpisode || '' });
        }
    ...
    return { exceptions, inferredDurations, suspectTitles, parseIssues };
  }
  ...
  function publishabilityFor(parsed) {
    ...
    // v1.4.21: do not block normal ProTrack/PDF extraction warnings.
    // The v1.4.20 gate was too aggressive: sparse rows and multiple episode tokens are
    // common in real reports, especially when accessibility/service codes are present.
    // Keep blocking only conditions that would clearly make the published month unusable
    // or visibly corrupt, and surface the rest as review warnings.
    ...
    const hardParseIssues = (diagnostics.parseIssues || []).filter(row => /episode-without-title|multiple-episode-tokens|title-sanitized-away/i.test(row.issue || ''));
    ...
    if (hardParseIssues.length) warnings.push(`${hardParseIssues.length} parser alignment issue(s) flagged for review`);
