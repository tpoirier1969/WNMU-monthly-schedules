...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.14-global-duration-alignment';
  const state = { parsed: {} };
  ...
  function durationTokens(text) {
    return Array.from(cleanupCellText(text).matchAll(/\b(\d{2}):(\d{2}):(\d{2})\b/g)).map(match => match[0]);
  }
  ...
  function inferSpan(title, episodes, durations) {
    return Math.max(episodeTokens(episodes).length, durationTokens(durations).length, 1);
  }
  ...
  function parseRowTriplet(titleLine, episodeLine, durationLine) {
    ...
    for (let i = 0; i < coarseCount; i += 1) {
      ...
      const durationList = durationTokens(durationParts[i]);
      while (durationList.length < span) durationList.push('');
      for (let j = 0; j < span; j += 1) {
        const durationToken = durationList[j] || '';
        cells.push({
          title: titles[j] || '',
          episode: episodes[j] || '',
          sourceStartTime: '',
          sourceEndTime: '',
          sourceDurationMin: durationMinutesFromToken(durationToken),
          sourceDurationToken: durationToken
        });
      }
    }
    while (cells.length < 7) cells.push({ title: '', episode: '', sourceStartTime: '', sourceEndTime: '', sourceDurationMin: null, sourceDurationToken: '' });

    // Global duration alignment:
    // Some ProTrack PDF rows do not keep duration tokens inside the same pipe cell
    ...
    const globalDurations = durationTokens(durationLine);
    const activeCells = cells.filter(cell => cell.title || cell.episode);
    if (globalDurations.length >= Math.max(1, Math.floor(activeCells.length * 0.65))) {
      let durationIndex = 0;
      for (const cell of cells) {
        if (!(cell.title || cell.episode)) continue;
        const token = globalDurations[durationIndex] || '';
        if (token) {
          cell.sourceDurationToken = token;
          cell.sourceDurationMin = durationMinutesFromToken(token);
        }
        durationIndex += 1;
      }
    }
      ...
      if (/^\|\s*\d{2}:\d{2}\s*\|/.test(lines[i])) {
        ...
        parsed.cells.forEach((cell, idx) => {
          if (cell.title || cell.episode) {
            entries.push({ date: weekDates[idx], time: parsed.rowTime, title: cell.title, episode: cell.episode, sourceStartTime: cell.sourceStartTime || '', sourceEndTime: cell.sourceEndTime || '', sourceDurationMin: cell.sourceDurationMin || null, sourceDurationToken: cell.sourceDurationToken || '' });
          }
        });
        i = nextIndex - 1;
      }
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    for (let offset = 0; offset < totalDays; offset += 1) {
      ...
      const normalizedEntries = deduped.map((entry, idx) => {
        ...
        const explicitLooksUsable = explicitDuration >= 30 && explicitDuration % 30 === 0;
        const maxUntilNextProgram = Math.max(30, nextStart - start);

        // Hybrid rule:
        // 1) Trust parsed duration tokens when available.
        // 2) If the duration token is missing, infer to the next parsed program
        //    like the older working behavior, but mark it as inferred so the
        //    summary tells Tod exactly where the parser had to guess.
        const durationMin = explicitLooksUsable ? Math.min(explicitDuration, maxUntilNextProgram) : maxUntilNextProgram;
        ...
        return { date: dateKey, day: dayName, dayName, time: entry.time, title: entry.title, episode: entry.episode, seasonStart: inferSeasonStart(entry.episode), endTime, durationMin: slotCount * 30, slotCount, sourceDurationMin: entry.sourceDurationMin || null, sourceDurationToken: entry.sourceDurationToken || '', durationSource: explicitLooksUsable ? 'explicit-pdf-duration-token' : 'inferred-to-next-program', inferredDuration };
      });
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.14 global duration alignment', days, weeks };
  ...
  function publishabilityFor(parsed) {
    ...
    if (diagnostics.inferredDurations.length > Math.max(40, expected * 3)) fatal.push(`${diagnostics.inferredDurations.length} program(s) still used inferred duration; duration parsing is not reliable enough yet`);
    else if (diagnostics.inferredDurations.length) warnings.push(`${diagnostics.inferredDurations.length} program(s) used inferred duration because no duration token was parsed`);
