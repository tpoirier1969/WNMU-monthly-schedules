(function () {
  ...
  const PACKAGE_VERSION = 'v1.5.78';
  const PACKAGE_LABEL = 'visible custom planning labels';
  const KNOWN_PARTS = [
    { key: 'package', label: 'Delivered package', version: `${PACKAGE_VERSION} ${PACKAGE_LABEL}` },
    { key: 'home', label: 'Home page shell', version: 'index.html v1.5.78' },
    { key: 'schedule-shell', label: 'Schedule page shell', version: 'index131/133 HTML v1.4.1 + package v1.5.78' },
    { key: 'builder-shell', label: 'Month Builder shell', version: 'month-builder HTML v1.4.1 / parser v1.4.16' },
    { key: 'interactions', label: 'Cell interactions / box notes', global: 'WNMU_CELL_INTERACTIONS_VERSION', fallback: 'v1.5.78 custom planning labels' },
    ...
    { key: 'framework', label: 'Fixed framework / scroll area', global: 'WNMU_FIXED_FRAMEWORK_VERSION', fallback: 'v1.5.78 custom planning labels layout' },
  ...
  function pageShellVersion() {
    ...
    if (page === 'index.html' || !page.includes('.html')) return 'Home page shell v1.5.78';
    if (/index13[13]\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.78`;
    if (/month-builder\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.78`;
    return `${page} + package v1.5.78`;
  }
