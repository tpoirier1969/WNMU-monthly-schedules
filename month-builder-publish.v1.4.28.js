...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.28-positioned-day-safe-episode-row-guard';
  const state = { parsed: {} };
  ...
  const ANCILLARY_CODES = new Set(['APX', 'DVS', 'STE', 'SAP', 'SPA', 'CC', 'HD', 'HDTV', 'VDS', 'VOD']);
  const ANCILLARY_CODE_RE = /\b(?:APX|DVS|STE|SAP|SPA|CC|HD|HDTV|VDS|VOD)\b/g;
  function stripEpisodeTokens(text) {
    return cleanupCellText(text)
      .replace(/#\s*[A-Za-z0-9$#]+/g, ' ')
      .replace(/\b\d{2}:\d{2}:\d{2}\s+\d{2}:\d{2}:\d{2}\b/g, ' ');
  }
  function stripAncillaryCodes(text) {
    return cleanupCellText(text).replace(ANCILLARY_CODE_RE, ' ').replace(/\s+/g, ' ').trim();
  }
  function titleSignalText(text) {
    return stripEpisodeTokens(stripAncillaryCodes(text))
      .replace(/[|\s0-9:;.,/\-_$#&()+]+/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  }
  function collapseRepeatedWordSequence(text) {
    let s = cleanupCellText(text);
    if (!s) return '';
    const tokens = s.split(/\s+/).filter(Boolean);
    if (tokens.length < 4) return s;

    // Collapse exact repeated phrases inside one PDF cell, e.g.
    // "CARL THE COLLECTOR CARL THE COLLECTOR CARL THE COLLECTOR".
    // This should never become one giant bogus title in one half-hour block.
    for (let size = 2; size <= Math.floor(tokens.length / 2); size += 1) {
      if (tokens.length % size !== 0) continue;
      const phrase = tokens.slice(0, size);
      if (!hasRealTitleSignal(phrase.join(' '))) continue;
      let ok = true;
      for (let pos = size; pos < tokens.length; pos += size) {
        const chunk = tokens.slice(pos, pos + size);
        if (chunk.length !== phrase.length || chunk.some((tok, idx) => tok !== phrase[idx])) { ok = false; break; }
      }
      if (ok) return phrase.join(' ');
    }

    // Also collapse a repeated prefix when a stray code/episode tail prevents exact divisibility.
    // Example: "TITLE TITLE TITLE SAP" -> "TITLE" after codes are stripped later.
    for (let size = 2; size <= Math.floor(tokens.length / 2); size += 1) {
      const phrase = tokens.slice(0, size);
      if (!hasRealTitleSignal(phrase.join(' '))) continue;
      let pos = size;
      let repeats = 1;
      while (pos + size <= tokens.length) {
        const chunk = tokens.slice(pos, pos + size);
        if (chunk.some((tok, idx) => tok !== phrase[idx])) break;
        repeats += 1;
        pos += size;
      }
      if (repeats >= 2) {
        const tail = tokens.slice(pos).join(' ');
        const tailSignal = titleSignalText(tail);
        if (!tailSignal || !hasRealTitleSignal(tailSignal)) return phrase.join(' ');
      }
    }
    return s;
  }

  function hasRealTitleSignal(text) {
    const signal = titleSignalText(text);
    if (!/[A-Za-z]/.test(signal)) return false;
    return signal.split(/\s+/).some(word => word.length > 1 && !ANCILLARY_CODES.has(word.toUpperCase()));
  }
  function isAncillaryOnlyLine(line) {
    const cleaned = cleanupCellText(line);
    if (!cleaned || !ANCILLARY_CODE_RE.test(cleaned)) { ANCILLARY_CODE_RE.lastIndex = 0; return false; }
    ANCILLARY_CODE_RE.lastIndex = 0;
    const leftover = stripAncillaryCodes(cleaned)
      .replace(/[|\s0-9:;.,/\-_$#&()+]+/g, '')
      .trim();
  ...
  function sanitizeTitleCandidate(text) {
    let s = cleanupCellText(text);
    if (!s) return '';
    // Episode/service-code fragments such as "#1412H STE #1413H DVS" are not titles.
    if (!hasRealTitleSignal(s)) return '';
    s = stripEpisodeTokens(s);
    s = stripAncillaryCodes(s);
    s = collapseRepeatedWordSequence(s);
    s = s.replace(/\s+/g, ' ').trim();
    if (!hasRealTitleSignal(s)) return '';
  ...
  function fixedPipeCells(line) {
    ...
    parts = parts.map(part => cleanupCellText(part));
    while (parts.length < 8) parts.push('');
    return parts.slice(0, 8);
  }
  function pipeLineFromCells(cells) {
    const fixed = (cells || []).slice(0, 8);
    while (fixed.length < 8) fixed.push('');
    return `| ${fixed.map(cell => cleanupCellText(cell)).join(' | ')} |`;
  }
  function mergePipeLineBuffers(baseLine, addLine) {
    const base = fixedPipeCells(baseLine);
    const add = fixedPipeCells(addLine);
    if (!base || !add) return `${baseLine || ''} ${addLine || ''}`.trim();
    const merged = base.map((cell, idx) => {
      const incoming = add[idx] || '';
      if (!incoming) return cell || '';
      if (!cell) return incoming;
      if (idx === 0 && /^\d{2}:\d{2}$/.test(cell)) return cell;
      return `${cell} ${incoming}`.replace(/\s+/g, ' ').trim();
    });
    return pipeLineFromCells(merged);
  }
  function isSeparatorLine(line) { return /^\|-+/.test(String(line || '').trim()); }
  function isTimeRowLine(line) { return /^\|\s*\d{2}:\d{2}\s*\|/.test(String(line || '').trim()); }
  function isDurationLine(line) { return durationStartPairs(line).length > 0; }
  function isEpisodeOnlyLine(line) {
    const cleaned = cleanupCellText(line);
    if (!/#\s*[A-Za-z0-9$#]+/.test(cleaned)) return false;
    const leftover = stripEpisodeTokens(stripAncillaryCodes(cleaned))
      .replace(/[|\s0-9:;.,/\-_$&()+]+/g, '')
      .trim();
    return leftover.length === 0;
  }
  function looksLikeTitleContinuation(line) {
    const cleaned = cleanupCellText(line);
    if (!cleaned || isSeparatorLine(cleaned) || isTimeRowLine(cleaned) || isDurationLine(cleaned) || isEpisodeOnlyLine(cleaned) || isAncillaryOnlyLine(cleaned)) return false;
    return hasRealTitleSignal(cleaned);
  }
  ...
  function uniqueEpisodeTokens(tokens) {
    const seen = new Set();
    const out = [];
    (tokens || []).forEach(token => {
      const clean = cleanupCellText(token).replace(/\s+/g, '');
  ...
  function titleLooksLikeSingleProgram(text) {
    const cleaned = sanitizeTitleCandidate(text);
    if (!cleaned) return false;
    const collapsed = collapseRepeatedWordSequence(cleaned);
    return !!collapsed && collapsed.split(/\s+/).filter(Boolean).length >= 2;
  }
  function inferSpan(title, episodes, durations) {
    const durationCount = durationStartPairs(durations).length;
    const episodeCount = episodeTokens(episodes).length;
    const titleEpisodeCount = episodeTokens(title).length;

    // Do not let multiple episode tokens embedded in one natural-language title
    // expand one day cell into several neighboring day cells. That was the source
    // of earlier column shifts and schedule holes. Duration/start pairs are still
    // the only strong signal that one coarse cell actually contains multiple airings.
    if (durationCount > 1) return durationCount;
    if (episodeCount > 1 && !titleLooksLikeSingleProgram(title)) return episodeCount;
    if (titleEpisodeCount > 1 && !titleLooksLikeSingleProgram(title)) return titleEpisodeCount;
    return 1;
  }
  function chooseEpisodesForCell(titleText, episodeText, span) {
    const fromEpisodeLine = uniqueEpisodeTokens(episodeTokens(episodeText));
    const fromTitleLine = uniqueEpisodeTokens(episodeTokens(titleText));
    let chosen = [];
    if (fromEpisodeLine.length) chosen = fromEpisodeLine.slice(0, span);
    if (chosen.length < span) {
      for (const token of fromTitleLine) {
        if (chosen.length >= span) break;
        if (!chosen.includes(token)) chosen.push(token);
      }
    }
    while (chosen.length < span) chosen.push('');
    return chosen;
  }
  function splitTitleCell(text, span) {
    ...
    // Only split when the PDF cell clearly repeats the same title text for multiple episodes.
    // Do not chop one natural-language title into fake fragments because the episode row
    // contains multiple #1234H tokens; duplicating the full title is much safer.
    for (let size = 1; size <= Math.floor(tokens.length / span); size += 1) {
      const pattern = tokens.slice(0, size);
      if (!pattern.length || !hasRealTitleSignal(pattern.join(' '))) continue;
      let ok = true;
  ...
  function stableDayParts(line) {
    const raw = String(line || '');
    const pipeCount = (raw.match(/\|/g) || []).length;
    if (pipeCount < 9) return null;
    const fixed = fixedPipeCells(raw);
    if (!fixed || fixed.length < 8) return null;
    return fixed.slice(1, 8);
  }
  function coarseCellHasData(titleText, episodeText, durationText) {
    return !!(cleanupCellText(titleText) || cleanupCellText(episodeText) || cleanupCellText(durationText));
  }
  ...
  function parsedCellFromParts(title, episode, pair) {
    return {
      title: title || '',
      episode: episode || '',
  ...
  function parseRowTripletAnchored(titleLine, episodeLine, durationLine) {
    const rowTime = coarseParts(titleLine, false)[0];
    const titleParts = stableDayParts(titleLine);
    const episodeParts = stableDayParts(episodeLine) || Array(7).fill('');
    const durationParts = stableDayParts(durationLine) || Array(7).fill('');
    if (!titleParts) return null;

    const cells = Array.from({ length: 7 }, () => blankParsedCell());
    const explicit = Array.from({ length: 7 }, (_, idx) => coarseCellHasData(titleParts[idx], episodeParts[idx], durationParts[idx]));

    for (let i = 0; i < 7; i += 1) {
      if (!explicit[i]) continue;
      const requestedSpan = Math.max(1, inferSpan(titleParts[i], episodeParts[i], durationParts[i]));

      // A merged ProTrack cell can legitimately represent several consecutive days.
      // Fill only the current day plus following *blank* coarse day slots. Never insert
      // cells into the row, because insertion shifts later explicit programs to the
      // wrong weekday and can push Saturday off the seven-day grid.
      const targetSlots = [i];
      for (let k = i + 1; k < 7 && targetSlots.length < requestedSpan; k += 1) {
        if (explicit[k]) break;
        targetSlots.push(k);
      }
      const span = targetSlots.length;
      const episodes = chooseEpisodesForCell(titleParts[i], episodeParts[i], span);
      ...
      targetSlots.forEach((slot, j) => {
        // An explicit program already anchored in this weekday always wins. This is
        // mostly defensive because targetSlots stops before explicit neighbors.
        if (slot !== i && explicit[slot]) return;
        cells[slot] = parsedCellFromParts(titles[j] || '', episodes[j] || '', pairList[j] || null);
      });
    }

    const globalPairs = durationStartPairs(durationLine);
    const activeIndexes = cells.map((cell, idx) => (cell.title || cell.episode) ? idx : -1).filter(idx => idx >= 0);
    if (globalPairs.length >= Math.max(1, Math.floor(activeIndexes.length * 0.65))) {
      activeIndexes.forEach((idx, pairIndex) => {
  ...
  function parseRowTripletLegacy(titleLine, episodeLine, durationLine) {
    const rowTime = coarseParts(titleLine, false)[0];
    ...
    for (let i = 0; i < coarseCount; i += 1) {
      ...
      const pairList = durationStartPairs(durationParts[i]);
      while (pairList.length < span) pairList.push(null);
      for (let j = 0; j < span; j += 1) cells.push(parsedCellFromParts(titles[j] || '', episodes[j] || '', pairList[j] || null));
    }
  ...
  function parseRowTriplet(titleLine, episodeLine, durationLine) {
    return parseRowTripletAnchored(titleLine, episodeLine, durationLine) || parseRowTripletLegacy(titleLine, episodeLine, durationLine);
  }
  function gatherRowBuffers(lines, startIndex) {
    let titleBuffer = String(lines[startIndex] || '').trim();
    let i = startIndex + 1;

    // ProTrack/PDF.js frequently emits long titles as a second sparse pipe row,
    // especially in the right-side weekday columns. Merge those continuation
    // rows by column instead of stopping early or appending them as fresh cells.
    // This prevents Saturday programs from sliding into earlier weekdays when
    // blank columns are lost in text-flow extraction.
    while (i < lines.length) {
      const line = String(lines[i] || '').trim();
      if (!line) { i += 1; continue; }
      if (isSeparatorLine(line) || isTimeRowLine(line) || isDurationLine(line) || isEpisodeOnlyLine(line)) break;
      if (!looksLikeTitleContinuation(line)) break;
      titleBuffer = mergePipeLineBuffers(titleBuffer, line);
      i += 1;
    }
    ...
    while (i < lines.length) {
      const line = String(lines[i] || '').trim();
      if (!line) { i += 1; continue; }
      if (isSeparatorLine(line) || isTimeRowLine(line) || isDurationLine(line)) break;
      if (looksLikeTitleContinuation(line) && !isEpisodeOnlyLine(line)) {
        titleBuffer = mergePipeLineBuffers(titleBuffer, line);
        i += 1;
        continue;
      }
      episodeBuffer = episodeBuffer ? mergePipeLineBuffers(episodeBuffer, line) : line;
      i += 1;
    }
    ...
    while (i < lines.length) {
      const line = String(lines[i] || '').trim();
      if (!line) { i += 1; continue; }
      if (isSeparatorLine(line) || isTimeRowLine(line)) break;
      durationBuffer = durationBuffer ? mergePipeLineBuffers(durationBuffer, line) : line;
      i += 1;
    }
  ...
  function parseReportPages(pages) {
    ...
    for (const page of pages) {
      ...
      const bansemer = pageEntries
        .filter(entry => /bansemer/i.test(`${entry.title} ${entry.episode}`))
        .map(entry => `${entry.date} ${entry.time} ${entry.title}`);
      debugWeeks.push({ page: page.pageNumber, weekStart: isoDate(pageWeekStart), rows: pageEntries.length, explicit: !!explicitWeekStart, extractionMode: page.extractionMode || '', columnSource: page.columnSource || '', bansemer });
    }
  ...
  function groupItemsByY(items) {
    const usable = items.map(item => ({ text: normalizeText(item.str || '').trim(), x: Number(item.transform?.[4] ?? 0), y: Number(item.transform?.[5] ?? 0), width: Number(item.width ?? 0) })).filter(item => item.text);
  ...
  function clusterNumbers(values, tolerance = 3.5) {
    const sorted = values.filter(value => Number.isFinite(value)).slice().sort((a, b) => a - b);
    const clusters = [];
    sorted.forEach(value => {
      const last = clusters[clusters.length - 1];
      if (!last || Math.abs(last.avg - value) > tolerance) clusters.push({ avg: value, values: [value] });
      else { last.values.push(value); last.avg = last.values.reduce((sum, item) => sum + item, 0) / last.values.length; }
    });
    return clusters.map(cluster => cluster.avg);
  }
  function inferWeekdayHeaderBoundaries(groups, minTimeX) {
    const weekdayNames = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
    let best = null;
    for (const group of groups) {
      const anchors = Array(7).fill(null);
      for (const item of group.items) {
        const label = cleanupCellText(item.text).toLowerCase();
        const idx = weekdayNames.indexOf(label);
        if (idx < 0) continue;
        // PDF.js exposes text width. Day headings are centered in their printed
        // columns, so the text midpoint is a much stronger anchor than program text.
        anchors[idx] = item.x + (Number.isFinite(item.width) && item.width > 0 ? item.width / 2 : 0);
      }
      const found = anchors.filter(value => Number.isFinite(value)).length;
      if (found < 7) continue;
      let monotonic = true;
      for (let idx = 1; idx < 7; idx += 1) {
        if (!(anchors[idx] > anchors[idx - 1])) { monotonic = false; break; }
      }
      if (!monotonic) continue;
      const spacings = anchors.slice(1).map((value, idx) => value - anchors[idx]);
      const minSpacing = Math.min(...spacings);
      const maxSpacing = Math.max(...spacings);
      if (!(minSpacing > 10) || maxSpacing / minSpacing > 1.45) continue;
      const boundaries = Array(8).fill(0);
      boundaries[0] = anchors[0] - spacings[0] / 2;
      for (let idx = 1; idx < 7; idx += 1) boundaries[idx] = (anchors[idx - 1] + anchors[idx]) / 2;
      boundaries[7] = anchors[6] + spacings[5] / 2;
      if (boundaries[0] <= minTimeX + 15) continue;
      const widths = boundaries.slice(1).map((value, idx) => value - boundaries[idx]);
      best = {
        minTimeX,
        minDayX: boundaries[0],
        maxX: boundaries[7],
        dayWidth: widths.reduce((sum, value) => sum + value, 0) / widths.length,
        boundaries,
        source: 'weekday-header'
      };
      break;
    }
    return best;
  ...
  function inferColumnBoundaries(groups) {
    const timeXs = [], rowItems = [], barXs = [];
    groups.forEach(group => {
      const timeItem = group.items.find(item => /^\d{2}:\d{2}$/.test(item.text));
      if (timeItem) {
        timeXs.push(timeItem.x);
        group.items.forEach(item => {
          rowItems.push(item);
          if (item.text === '|') barXs.push(item.x);
        });
      }
    });
    const minTimeX = timeXs.length ? Math.min(...timeXs) : 35;
    const weekdayHeader = inferWeekdayHeaderBoundaries(groups, minTimeX);
    if (weekdayHeader) return weekdayHeader;
    const bars = clusterNumbers(barXs).filter(x => x > minTimeX + 18).sort((a, b) => a - b);
    if (bars.length >= 8) {
      // The report's printed vertical rules are better weekday anchors than a
      // calculated equal-width grid. Keep exactly eight day boundaries (left edge
      // of Sunday through right edge of Saturday) and assign text by interval.
      let boundaries;
      if (bars.length === 8) boundaries = bars.slice();
      else boundaries = Array.from({ length: 8 }, (_, idx) => bars[Math.round(idx * (bars.length - 1) / 7)]);
      const widths = boundaries.slice(1).map((x, idx) => x - boundaries[idx]).filter(width => width > 0);
      const minWidth = widths.length ? Math.min(...widths) : 0;
      const maxWidth = widths.length ? Math.max(...widths) : 0;
      if (widths.length === 7 && minWidth > 10 && (!maxWidth || maxWidth / minWidth < 1.7)) {
        return { minTimeX, minDayX: boundaries[0], maxX: boundaries[7], dayWidth: widths.reduce((a, b) => a + b, 0) / widths.length, boundaries, source: 'bar-grid' };
      }
    }
    const allXs = rowItems.filter(item => item.text !== '|').map(item => item.x).filter(x => x > minTimeX + 25);
    const minDayX = allXs.length ? Math.min(...allXs) : minTimeX + 55;
    const maxX = allXs.length ? Math.max(...allXs) + 80 : 780;
    const dayWidth = Math.max(1, (maxX - minDayX) / 7);
    return { minTimeX, minDayX, maxX, dayWidth, boundaries: null, source: 'text-range' };
  }
  function dayColumnForX(x, cols) {
    if (Array.isArray(cols.boundaries) && cols.boundaries.length === 8) {
      if (x < cols.boundaries[0]) return 1;
      for (let idx = 0; idx < 7; idx += 1) {
        if (x >= cols.boundaries[idx] && x < cols.boundaries[idx + 1]) return idx + 1;
      }
      return 7;
    }
    return Math.max(1, Math.min(7, Math.floor((x - cols.minDayX) / cols.dayWidth) + 1));
  }
  function isMergedRunText(text) {
    const cleaned = cleanupCellText(text);
    if (!cleaned) return false;
    if (episodeTokens(cleaned).length > 1 || durationStartPairs(cleaned).length > 1) return true;
    const titleOnly = stripAncillaryCodes(stripEpisodeTokens(cleaned)).replace(/\s+/g, ' ').trim();
    if (!titleOnly) return false;
    const collapsed = collapseRepeatedWordSequence(titleOnly);
    return !!collapsed && collapsed !== titleOnly;
  }
  function dayColumnForItem(item, cols) {
    const x = Number(item?.x ?? 0);
    const width = Number(item?.width ?? 0);

    // A single title can be centered in its printed cell and therefore START to
    // the left of that cell's boundary. Use its text midpoint to identify the day.
    // Merged multi-day runs (repeated titles / episode lists / duration lists) are
    // different: their midpoint spans several days, so those must stay anchored
    // by their left edge and be expanded later by parseRowTripletAnchored().
    const anchorX = width > 0 && !isMergedRunText(item?.text) ? x + width / 2 : x;
    let col = dayColumnForX(anchorX, cols);
    if (!Array.isArray(cols.boundaries) || cols.boundaries.length !== 8 || col >= 7) return col;

    // Tiny PDF positioning drift can still place a run a few points before the
    // next boundary. Snap forward only when the text actually crosses it.
    const boundary = cols.boundaries[col];
    const avgWidth = Number(cols.dayWidth || 0);
    const tolerance = Math.max(3, Math.min(8, avgWidth * 0.06));
    if (anchorX < boundary && boundary - anchorX <= tolerance && width > 0 && x + width * 0.2 >= boundary) col += 1;
  ...
  function lineToPipe(group, cols) {
    ...
    group.items.forEach(item => {
      ...
      const col = dayColumnForItem(item, cols);
  ...
  function extractLinesByPosition(items) {
    ...
    groups.forEach(group => {
      ...
      else pushLine(lines, group.items.filter(item => item.text !== '|').map(item => item.text).join(' '));
    });
    return { lines, columnSource: cols.source || 'unknown', dayWidth: cols.dayWidth };
  }
  async function extractPdfPages(file) {
    ...
    for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
      ...
      const positioned = extractLinesByPosition(tc.items);
      const positionedLines = positioned.lines || [];
      const eolRows = countTimeRows(eolLines);
      const positionedRows = countTimeRows(positionedLines);

      // CRITICAL: never use ProTrack's text-flow table to assign weekdays when
      // positioned PDF text is available. The report uses merged/colspan cells,
      // so empty weekday columns disappear from text flow. A real Saturday item
      // can then look like the 3rd/4th sequential cell and be misread as Tuesday
      // or Wednesday. Positioned extraction preserves the printed X coordinate.
      //
      // Keep a few text-flow metadata lines only to improve FROM:/week detection;
      // all schedule rows themselves come from positioned extraction.
      if (positionedRows > 0) {
        const metadata = eolLines.filter(line => /\bFROM\s*:|\bWEEK\s+OF\b|REPORT DATE|REPORT TIME/i.test(line));
        const lines = [...metadata, ...positionedLines];
        const mode = positionedRows < eolRows ? 'positioned-day-safe+text-metadata' : 'positioned-day-safe';
        pages.push({ pageNumber, lines, eolTimeRows: eolRows, positionedTimeRows: positionedRows, extractionMode: mode, columnSource: positioned.columnSource || 'unknown' });
        continue;
      }

      // If PDF.js cannot provide positioned schedule rows, do not silently guess
      // weekday columns from collapsed text-flow cells. Preserve the page metadata
      // for diagnostics; publishability checks will expose the resulting missing
      // content instead of moving programs to the wrong days.
      const metadataOnly = eolLines.filter(line => /\bFROM\s*:|\bWEEK\s+OF\b|REPORT DATE|REPORT TIME/i.test(line));
      pages.push({ pageNumber, lines: metadataOnly, eolTimeRows: eolRows, positionedTimeRows: 0, extractionMode: 'positioned-unavailable-no-day-guess', columnSource: positioned.columnSource || 'unknown' });
    }
    return pages;
  }
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.28 positioned-day-safe non-strict recovery parser with episode retention and service-code guard', days, weeks };
  ...
  function collectCoverageDiagnostics(schedule, verification) {
    ...
    (schedule.days || []).forEach(day => {
      (day.entries || []).forEach(entry => {
        ...
        const signal = titleSignalText(entry.title);
        if (!signal || /#\s*[A-Za-z0-9$#]+/.test(entry.title || '') || /\b(?:APX|DVS|STE)\b/.test(entry.title || '')) {
          suspectTitles.push({ date: day.date, time: entry.time, title: entry.title || '', episode: entry.episode || '' });
        }
        if (!entry.episode) {
    ...
    return { exceptions, inferredDurations, suspectTitles, missingEpisodes };
  }
  ...
  function publishabilityFor(parsed) {
    ...
    const unsafePositionPages = (parsed.extractionDebug || []).filter(row => Number(row.eolTimeRows || 0) > 0 && Number(row.positionedTimeRows || 0) === 0);
    const partialPositionPages = (parsed.extractionDebug || []).filter(row => Number(row.positionedTimeRows || 0) > 0 && Number(row.positionedTimeRows || 0) < Number(row.eolTimeRows || 0));
    if (unsafePositionPages.length) {
      fatal.push(`${unsafePositionPages.length} page(s) contain schedule rows but no positioned PDF rows; refusing to guess weekday columns from collapsed text flow`);
    }
    if (partialPositionPages.length) {
      warnings.push(`${partialPositionPages.length} page(s) had fewer positioned time rows than text-flow rows; weekday placement stayed position-safe, so review any visible holes`);
    }

    // v1.4.22: rollback from the v1.4.20 strict-seven-cell parser, which caused
    // real programs to be dropped when ProTrack/PDF extraction packed multiple
    // airings or episode rows into one visual cell. Block only obviously unsafe
    // publishes; keep the rest as warnings for review.
    ...
    if ((diagnostics.suspectTitles || []).length) {
      fatal.push(`${diagnostics.suspectTitles.length} suspect title(s) look like episode/accessibility-code fragments`);
    }

    if ((parsed.entryCount || 0) < expected * 20) warnings.push(`only ${parsed.entryCount} imported airings; expected roughly ${expected * 20}+ for a full month`);
  ...
  function renderParsedSummary(channelCode, parsed) {
    ...
    const debugLines = (parsed.pageDebug || []).map(row => {
      const bansemer = Array.isArray(row.bansemer) && row.bansemer.length ? ` Bansemer=[${row.bansemer.join('; ')}]` : '';
      return `p${row.page}: ${row.weekStart || 'no week'} ${row.explicit ? 'explicit' : 'inferred'} rows=${row.rows} mode=${row.extractionMode || '?'} cols=${row.columnSource || '?'}${bansemer}`;
    }).join('\n');
    const extractionLines = (parsed.extractionDebug || []).map(row => `p${row.page}: eolRows=${row.eolTimeRows}, positionedRows=${row.positionedTimeRows}, mode=${row.mode || '?'}, columns=${row.columns || '?'}`).join('\n');
  ...
  async function parseSelectedReports() {
    ...
    if (blocked.length) {
      setStatus(`Parsed, but publish is BLOCKED because the import contains clear fatal problems:\n${blocked.map(code => `${code}: ${state.parsed[code].publishability.fatal.join('; ')}`).join('\n')}\n\nThis prevents clearly corrupt schedule data from being published. Nonfatal parser warnings are listed in the report for review.`, 'bad');
      return;
    }
