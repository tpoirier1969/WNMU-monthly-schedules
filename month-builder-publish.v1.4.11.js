...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.11-content-day-overview';
  const state = { parsed: {} };
  ...
  function durationMinutesFromPair(pair) {
    if (!pair || !pair[0] || !pair[1]) return null;
    const start = minutesFromTime(pair[0].slice(0, 5));
    let end = minutesFromTime(pair[1].slice(0, 5));
    if (end === 0 && pair[1].startsWith('00:00')) end = 1440;
  ...
  function parseRowTriplet(titleLine, episodeLine, durationLine) {
    ...
    for (let i = 0; i < coarseCount; i += 1) {
      ...
      const pairs = timePairs(durationParts[i]); while (pairs.length < span) pairs.push(null);
      for (let j = 0; j < span; j += 1) {
        const pair = pairs[j] || null;
        cells.push({
          title: titles[j] || '',
          episode: episodes[j] || '',
          sourceStartTime: pair?.[0] ? pair[0].slice(0, 5) : '',
          sourceEndTime: pair?.[1] ? pair[1].slice(0, 5) : '',
          sourceDurationMin: durationMinutesFromPair(pair)
        });
      }
    }
    while (cells.length < 7) cells.push({ title: '', episode: '', sourceStartTime: '', sourceEndTime: '', sourceDurationMin: null });
      ...
      if (/^\|\s*\d{2}:\d{2}\s*\|/.test(lines[i])) {
        ...
        parsed.cells.forEach((cell, idx) => {
          if (cell.title || cell.episode) {
            entries.push({ date: weekDates[idx], time: parsed.rowTime, title: cell.title, episode: cell.episode, sourceStartTime: cell.sourceStartTime || '', sourceEndTime: cell.sourceEndTime || '', sourceDurationMin: cell.sourceDurationMin || null });
          }
        });
        i = nextIndex - 1;
      }
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    for (let offset = 0; offset < totalDays; offset += 1) {
      ...
      const normalizedEntries = deduped.map((entry, idx) => {
        ...
        const explicitStartMatchesRow = !!sourceStartTime && sourceStartTime === entry.time;
        const explicitLooksUsable = explicitStartMatchesRow && explicitDuration >= 30 && explicitDuration % 30 === 0;
        const maxUntilNextProgram = Math.max(30, nextStart - start);

        // Conservative rule:
        // If the PDF did not give a matching explicit duration, do NOT stretch the
        // program forward to the next parsed program. Stretching hides real schedule
        // holes. Default to 30 minutes and leave any blank half-hour visible.
        ...
        return { date: dateKey, day: dayName, dayName, time: entry.time, title: entry.title, episode: entry.episode, seasonStart: inferSeasonStart(entry.episode), endTime, durationMin: slotCount * 30, slotCount, sourceStartTime: entry.sourceStartTime || '', sourceDurationMin: entry.sourceDurationMin || null, sourceEndTime: entry.sourceEndTime || '', durationSource: explicitLooksUsable ? 'explicit-pdf-timepair' : 'conservative-30min-default', hiddenHoleRisk };
      });
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.11 content-day overview', days, weeks };
      ...
      (day.entries || []).forEach(entry => {
        if (entry.hiddenHoleRisk) {
          hiddenRisks.push({
            ...
            reason: 'No matching PDF duration; left following time visible as hole instead of stretching program.'
          });
        }
      });
    });
  ...
  function publishabilityFor(parsed) {
    ...
    if (diagnostics.hiddenRisks.length) fatal.push(`${diagnostics.hiddenRisks.length} suspected hidden hole(s) / unverified program durations`);
