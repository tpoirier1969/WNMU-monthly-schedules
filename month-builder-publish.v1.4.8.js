...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.8-duration-hole-parser';
  const state = { parsed: {} };
      ...
      if (/^\|\s*\d{2}:\d{2}\s*\|/.test(lines[i])) {
        ...
        parsed.cells.forEach((cell, idx) => {
          if (cell.title || cell.episode) {
            entries.push({ date: weekDates[idx], time: cell.sourceStartTime || parsed.rowTime, title: cell.title, episode: cell.episode, sourceEndTime: cell.sourceEndTime || '', sourceDurationMin: cell.sourceDurationMin || null });
          }
        });
        i = nextIndex - 1;
      }
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    for (let offset = 0; offset < totalDays; offset += 1) {
      ...
      const normalizedEntries = deduped.map((entry, idx) => {
        ...
        const boundedExplicitDuration = explicitDuration > 0 ? Math.min(explicitDuration, Math.max(30, 1440 - start)) : 0;
        const inferredDuration = Math.max(30, nextStart - start);
        const durationMin = boundedExplicitDuration || inferredDuration;
        ...
        const endTime = entry.sourceEndTime || endTimeFromMinutes(start + slotCount * 30);
        return { date: dateKey, day: dayName, dayName, time: entry.time, title: entry.title, episode: entry.episode, seasonStart: inferSeasonStart(entry.episode), endTime, durationMin: slotCount * 30, slotCount, sourceDurationMin: entry.sourceDurationMin || null, sourceEndTime: entry.sourceEndTime || '' };
      });
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.8 duration/hole-aware parser', days, weeks };
