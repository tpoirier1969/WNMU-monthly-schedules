(function () {
  const VERSION = 'v1.4.30-blank-menu-color-fix';
  ...
  function injectStyles() {
    if (document.getElementById('wnmuBlankSlotMenuV1430Styles')) return;
    const style = document.createElement('style');
    style.id = 'wnmuBlankSlotMenuV1430Styles';
    style.textContent = `
  ...
  function markVersion() {
    ...
    if (!flag) return;
    if (!flag.textContent.includes('blank color fix 1.4.30')) {
      flag.textContent = `${flag.textContent} • blank color fix 1.4.30`;
    }
  }
