  ...
  async function buildUpdatedPackage() {
    ...
    try {
      const [registry, manifest] = await Promise.all([
        fetchJson('data/month-registry.v1.4.1.json'),
        fetchJson('site-manifest.v1.4.1.json')
      ]);
      ...
      parsedChannels.forEach(channelCode => {
        ...
        nextRegistry.channels[channelCode].months[parsed.monthKey] = {
          ...
          storageKey: `${channel.prefix}-${parsed.monthKey}-marks-v1.4.1`
        };
      ...
      for (const relativePath of manifest.files) {
        if (generatedFiles.has(relativePath) || relativePath === 'data/month-registry.v1.4.1.json' || relativePath === 'data/month-registry.v.1.4.1.json') continue;
      ...
      zip.file('data/month-registry.v1.4.1.json', JSON.stringify(nextRegistry, null, 2));
      zip.file('data/month-registry.v.1.4.1.json', JSON.stringify(nextRegistry, null, 2));
      ...
      const fileName = `WNMU-monthly-schedules-${monthBits.join('_')}-v1.4.1-no-config.zip`;
