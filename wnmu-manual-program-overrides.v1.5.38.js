(function () {
  'use strict';
  const VERSION = 'v1.5.38-true-imported-schedule-overwrite-soft-grid-refresh-soft-grid-refresh';
  const TABLE = 'wnmu_monthly_schedule_overrides'; // legacy only; v1.5.38 does not write or render override-table records.
  ...
  async function loadTableOverrides() {
    // v1.5.38: manual commits physically update imported month schedule_json.
  ...
  async function deactivateRow(row) { throw new Error('Remove committed program is disabled in v1.5.38 because commits now overwrite imported schedule_json. Restore from backup/reimport if a committed replacement needs to be undone.'); }
  ...
  function cloneWeekGridHostForRendererReset() {
    const oldHost = document.getElementById('weekGrids');
    if (!oldHost || !oldHost.parentNode) return null;
    const nextHost = oldHost.cloneNode(false);
    nextHost.id = oldHost.id;
    nextHost.className = oldHost.className;
    nextHost.setAttribute('aria-busy', 'true');
    nextHost.innerHTML = '<section class="week-grid-wrap"><div class="rollup-box"><div class="rollup-empty">Refreshing updated schedule grid from imported month data…</div></div></section>';
    oldHost.parentNode.replaceChild(nextHost, oldHost);
    return nextHost;
  }
  ...
  function softReloadSharedScheduleRenderer(row) {
    return new Promise((resolve, reject) => {
      try {
        ...
        clearJsonCachesForMonth(row?.channel_code || channelCode(), row?.month_key || monthKey() || String(row?.date || '').slice(0, 7));
        cloneWeekGridHostForRendererReset();
        ...
        script.onload = () => {
          window.setTimeout(() => {
            window.dispatchEvent(new CustomEvent('wnmu:schedule-grid-soft-refreshed', { detail: { version: VERSION, row } }));
            finish(true);
          }, 0);
        };
  ...
  async function commitFromMenu(menu) {
    ...
    try {
      ...
      menuStatus(menu, `Imported schedule data overwritten and schedule grid refreshed. Replaced ${committed.replaced_count || 0} imported block(s); preserved ${committed.preserved_split_count || 0} split piece(s).`);
    } catch (err) {
      menuStatus(menu, err.message || String(err), true);
