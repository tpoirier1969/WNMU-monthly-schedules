(function () {
  'use strict';
  const VERSION = 'v1.5.77-performance-safe-custom-planning-and-season-start';
  ...
  function tagLabel(tag, otherLabel = ''){ if (tag === OTHER_KEY) return cleanOtherLabel(otherLabel) || 'Custom'; return tagMeta()[tag]?.label || (tag === SATELLITE_KEY ? 'Satellite Feed' : tag); }
  ...
  function writeCustomOtherLabelToMenu(menu, label){ const clean = cleanOtherLabel(label); if (menu) menu.dataset.wnmuOtherLabel = clean; const input = customTagInput(menu); if (input && input.value !== clean) input.value = clean; }
  function canonicalStoreKey(){ return storageKey() ? `${storageKey()}::cellOverrides.v1.5.0` : ''; }
  function otherLabelFromRecord(record){ return cleanOtherLabel(record?.otherLabel || record?.planningOtherLabel || ''); }
  ...
  function renderTags(cell, tags){
    ...
    const signature = active.join('|') + `::sat=${isSatelliteOnly ? '1' : '0'}::other=${otherLabel}`;
    ...
    if (content) {
      ...
      wrap.innerHTML = pills.map(tag => `<span class="tag-pill" style="--tag-color:${colorFor(tag)}">${tagLabel(tag, otherLabel)}</span>`).join('');
  ...
  function populateMenuEffectiveTags(){
    ...
    if (menu) writeCustomOtherLabelToMenu(menu, otherLabel);
  ...
  function writeExplicitTagsForMenu(menu, tags){
    ...
    const writeRecord = (recordKey) => {
      ...
      const otherLabel = clean[OTHER_KEY] ? cleanOtherLabel(readCustomOtherLabelFromMenu(menu) || existing.otherLabel || existing.planningOtherLabel || '') : '';
      marks[recordKey] = { ...existing, tags: { ...clean }, otherLabel, planningOtherLabel: otherLabel };
    };
  ...
  function installImmediateMenuSync(){
    ...
    document.addEventListener('input', event => {
      ...
      const menu = input.closest && input.closest('#wnmuCellMenu');
      if (!menu) return;
      writeCustomOtherLabelToMenu(menu, input.value);
    ...
    document.addEventListener('click', event => {
      ...
      if (!menu) return;
      if (!target.closest('#wnmuCellSaveBtn, #wnmuCommitProgramBtn, #wnmuCellClearBtn, #wnmuCustomTagClearBtn')) return;
  ...
  function installObservers(){
    ...
    [600,1600,3200].forEach(ms => window.setTimeout(() => { populateMenuEffectiveTags(); sanitizeVisibleHighlights(); }, ms));
  }

  function start(){ installStorageDisableHook(); installRendererAppendHook(); installImmediateMenuSync(); installObservers(); }
