(function () {
  'use strict';
  const VERSION = 'v1.5.91-auto-tags-custom-grid-render-stability';
  ...
  function recordsForCell(cell){
    ...
    if (entryId) {
      Object.entries(main || {}).forEach(([recordKey, record]) => {
        if (!record || typeof record !== 'object') return;
        if (record.sourceEntryId === entryId || record.legacyEntryId === entryId) push(record);
        if (record.rectNote && typeof record.rectNote === 'object' && (record.rectNote.sourceEntryId === entryId || record.rectNote.legacyEntryId === entryId)) push(record);
        // Older saves could be stored only under the canonical date/time key, while the visible
        // program cell is keyed by entryId. Preserve those custom labels if they point at this entry.
        if (record.sourceEntryId && record.sourceEntryId === entryId) push(record);
      });
      Object.values(canonical || {}).forEach(record => {
        if (!record || typeof record !== 'object') return;
        if (record.sourceEntryId === entryId || record.legacyEntryId === entryId) push(record);
      });
    }
  ...
  function installObservers(){
    ...
    if ('MutationObserver' in window) {
      const bodyObs = new MutationObserver(mutations => {
        if (mutations.some(m => Array.from(m.addedNodes || []).some(n => n && n.nodeType === 1 && (n.id === 'wnmuCellMenu' || n.querySelector?.('#wnmuCellMenu'))))) scheduleCustomLabelRepair(80);
      });
