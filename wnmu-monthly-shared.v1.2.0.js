(function () {
  ...
  const DATA_FILES = { schedule: CONFIG.scheduleFile, verification: CONFIG.verificationFile };
  ...
  function mustGet(id) {
    ...
    if (!el) throw new Error(`Missing required element #${id}. Replace the entire site with the same package.`);
    return el;
  }
  ...
  function normalizeStoredState(raw) {
    ...
    if ('tags' in raw || 'rectNote' in raw || 'note' in raw || 'whiteout' in raw) {
      ...
      return { tags: pickBooleanTags(raw.tags || raw), rectNote: migratedRect };
    }
  ...
  function renderFlags() {
    const archiveBit = isArchiveMode ? ' • archive view' : ' • archive-ready';
    mustGet('versionFlag').textContent = `${BUILD_VERSION}${archiveBit}`;
  ...
  function renderWeekGrids() {
    ...
    appState.schedule.weeks.forEach((week, weekIndex) => {
      ...
      for (let slot = 0; slot < 48; slot++) {
        const tr = document.createElement('tr');
        const slotTime = slot % 2 === 0 ? `${String(Math.floor(slot / 2)).padStart(2, '0')}:00` : `${String(Math.floor(slot / 2)).padStart(2, '0')}:30`;
        tr.dataset.slotIndex = String(slot);
        const timeTd = document.createElement('td');
        timeTd.className = 'time-col';
        timeTd.textContent = fmtTime(slotTime);
        tr.appendChild(timeTd);
      ...
      wrap.appendChild(buildPrintWeekGrid(week));
  ...
  function beginRectDraw() {
    ...
    if (text === null) return;
    const defaultRect = existing ? { ...existing } : { x: 3, y: 4, w: 94, h: 90, text: '' };
    attachRectNote(entry, { ...defaultRect, text });
    updateRectControls(entry);
    closeContextMenu();
  }
  ...
  async function init() {
    const { schedule, verification } = await loadData();
