(function () {
  'use strict';
  const VERSION = 'v1.5.88-sponsor-guide-light-pattern-palette';
  ...
  function titleLine(entry) {
    // Sponsorship guide display: use program titles only; drop episode numbers.
    // Episode text remains available to classification rules through keywordRule/exactTitleOverride.
  ...
  async function optionalRest(pathAndQuery) {
    ...
    catch (err) { console.info(`${VERSION}: optional Supabase lookup skipped`, err && (err.message || err)); return null; }
  }
  ...
  function block(label, detail, cls, mode = 'topic') {
  ...
  function timeslotRule(entry, day, slot) {
    ...
    if (wi >= 0) {
      ...
      if (inRange(slot, '07:00', '08:00')) return block('Health / Wellness', shortTitle(entry), 'health');
      ...
      if (wi === 4 && inRange(slot, '20:30', '21:00')) return block('News / Public Affairs', '', 'news', 'block');
  ...
  function topicBlockKey(block) { return `${block.label}||${block.cls}`; }
  function isLiteralTitleBlock(block) { return block?.titleMode === 'title' || block?.cls === 'program'; }

  function canMergeDailyTopicBlocks(current, next) {
    if (!current || !next) return false;
    // Literal program-title fallback blocks stay separate unless they are truly identical.
    if (isLiteralTitleBlock(current) || isLiteralTitleBlock(next)) return blockKey(current) === blockKey(next);
    // Sponsorship guide rule: within one day column only, adjacent blocks with the same topic merge visually.
    return topicBlockKey(current) === topicBlockKey(next);
  }
  ...
  function appendSegmentDetail(segment, detail, slot) {
    if (!segment || !detail) return;
    const offset = Math.max(0, Number(slot) - Number(segment.startSlot || slot));
    splitDetailText(detail).forEach(text => {
      const existing = segment.detailItems.find(item => item.text === text);
      if (existing) {
        existing.offset = Math.min(existing.offset, offset);
        return;
      }
      segment.detailItems.push({ text, offset });
    });
    segment.detail = segment.detailItems.map(item => item.text).join(' • ');
  }

  function makeSegment(block, slot) {
    const segment = { ...block, startSlot: slot, endSlot: slot + 1, detailItems: [] };
    appendSegmentDetail(segment, block.detail, slot);
    return segment;
  }
  ...
  function daySegments(day) {
    ...
    for (let slot = START_SLOT; slot < END_SLOT; slot += 1) {
      ...
      if (!current || !canMergeDailyTopicBlocks(current, block)) {
        if (current) segments.push(current);
        current = makeSegment(block, slot);
      } else {
        current.endSlot = slot + 1;
        appendSegmentDetail(current, block.detail, slot);
      }
    }
    if (current) segments.push(current);
    return segments;
  ...
  function renderSegmentDetails(segment, span) {
    const items = Array.isArray(segment.detailItems) ? segment.detailItems.slice() : [];
    if (!items.length && segment.detail) {
      splitDetailText(segment.detail).forEach((text, offset) => items.push({ text, offset }));
    }
    if (!items.length) return '';
    items.sort((a, b) => a.offset - b.offset || a.text.localeCompare(b.text));
    return `<div class="sales-detail-list" style="--slot-span:${span}">` + items.map(item => {
      const row = Math.max(1, Math.min(span, Number(item.offset || 0) + 1));
      return `<div class="sales-detail-item" style="grid-row:${row}">${esc(item.text)}</div>`;
    }).join('') + '</div>';
  }
  ...
  function buildWeekTable(schedule, week, weekIndex, dayLookup) {
    ...
    for (let slot = START_SLOT; slot < END_SLOT; slot += 1) {
      ...
      week.forEach((day, dayIndex) => {
        ...
        const details = renderSegmentDetails(covering, span);
        html += `<td class="sales-block sales-${esc(covering.cls)}" rowspan="${span}" style="--slot-span:${span}"><div class="sales-block-inner"><div class="sales-label">${label}</div>${details}</div></td>`;
      });
      html += '</tr>';
    }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      :root {
        ...
        --sales-kids:#fff8c9; --sales-health:#e4f6ea; --sales-financial:#f3e4fb; --sales-news:#f1f4fb;
        --sales-food:#fde9d5; --sales-home:#f1ead9; --sales-travel:#def8f4; --sales-michigan:#e6f4ff; --sales-local:#e6f6df;
        --sales-arts:#eee5fb; --sales-environmental:#eef6d6; --sales-science:#e4fbff; --sales-history:#f8ead7; --sales-documentary:#edf0fb;
        --sales-drama:#fae5f0; --sales-talk:#f5edff; --sales-native:#f5e4d9; --sales-variety:#f7f2cf; --sales-choice:#eaf5d7; --sales-specials:#eceaf8; --sales-fundraiser:#fae3e7; --sales-program:#f5f7fa; --sales-blank:#ffffff;
      }
      * { box-sizing:border-box; }
      ...
      .sales-kids{background:var(--sales-kids)!important}.sales-health{background:var(--sales-health)!important}.sales-financial{background:var(--sales-financial)!important}.sales-food{background:var(--sales-food)!important}.sales-home{background:var(--sales-home)!important}.sales-travel{background:var(--sales-travel)!important}.sales-local{background:var(--sales-local)!important}.sales-native{background:var(--sales-native)!important}.sales-variety{background:var(--sales-variety)!important}.sales-choice{background:var(--sales-choice)!important}.sales-specials{background:var(--sales-specials)!important}.sales-fundraiser{background:var(--sales-fundraiser)!important}.sales-arts{background:var(--sales-arts)!important}.sales-environmental{background:var(--sales-environmental)!important}.sales-history{background:var(--sales-history)!important}.sales-documentary{background:var(--sales-documentary)!important}.sales-drama{background:var(--sales-drama)!important}.sales-program{background:var(--sales-program)!important}.sales-blank{background:var(--sales-blank)!important}
      .sales-news{background:repeating-linear-gradient(135deg, rgba(74,91,122,.13) 0 2px, transparent 2px 9px), var(--sales-news)!important}
      .sales-talk{background:radial-gradient(circle at 4px 4px, rgba(93,74,124,.16) 0 1.1px, transparent 1.4px) 0 0/9px 9px, var(--sales-talk)!important}
      .sales-michigan{background:repeating-linear-gradient(0deg, rgba(48,102,142,.13) 0 2px, transparent 2px 10px), var(--sales-michigan)!important}
      .sales-science{background:repeating-linear-gradient(90deg, rgba(35,115,130,.12) 0 2px, transparent 2px 10px), var(--sales-science)!important}
      ...
      .sales-block { padding:0; min-height:24px; overflow:hidden; }
      .sales-block-inner { min-height:calc(var(--slot-span,1) * 42px); padding:5px 6px; display:flex; flex-direction:column; gap:2px; }
      .sales-label { font-weight:950; font-size:12px; color:#000; letter-spacing:-.01em; }
      .sales-detail { margin-top:3px; font-size:10.5px; color:#49566a; opacity:.78; line-height:1.18; }
      .sales-detail-list { flex:1 1 auto; display:grid; grid-template-rows:repeat(var(--slot-span,1), minmax(14px,1fr)); gap:0; margin-top:2px; }
      .sales-detail-item { font-size:9.8px; color:#49566a; opacity:.86; line-height:1.08; align-self:start; overflow:hidden; text-transform:none; }
      .sales-program .sales-label { color:#8b94a3; font-weight:800; }
      .sales-range { display:none; }
      ...
      @media print {
        ...
        .sales-block-inner { min-height:calc(var(--slot-span,1) * 28px); padding:3px 4px; }
        .sales-label { font-size:9px; }
        .sales-detail { font-size:7.6px; }
        .sales-detail-list { grid-template-rows:repeat(var(--slot-span,1), minmax(10px,1fr)); }
        .sales-detail-item { font-size:7.2px; line-height:1.02; }
        .sales-range { display:none; }
      }
    `;
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.88` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.88`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.88 note: sales category colors use a lighter, print-friendlier palette with subtle pattern help for similar categories; fundraiser detection still uses only exact date/time placements from the actual Fundraising Schedule.</small></div>`;
    }
  }
