(function () {
  'use strict';
  const VERSION = 'v1.5.26-explicit-commit-manual-program-overrides';
  ...
  async function loadTableOverrides() {
    ...
    const clean = Array.isArray(rows) ? rows.filter(row => row && row.title_text) : [];
  ...
  function ensureCommitButton(menu) { if (!menu || menu.querySelector('#wnmuCommitProgramBtn')) return; const form = menu.querySelector('#wnmuCellForm') || menu.querySelector('form') || menu; const actions = document.createElement('div'); actions.className = 'rect-actions wnmu-commit-actions'; actions.style.marginTop = '8px'; actions.innerHTML = '<button type="button" class="btn ghost" id="wnmuCommitProgramBtn">Commit to Schedule</button><button type="button" class="btn ghost" id="wnmuRemoveCommittedProgramBtn" hidden>Remove committed program</button>'; form.appendChild(actions); actions.querySelector('#wnmuCommitProgramBtn')?.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); commitFromMenu(menu); }); actions.querySelector('#wnmuRemoveCommittedProgramBtn')?.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); removeFromMenu(menu); }); prefillCommitted(menu); }
  ...
  function start() { installMenuObserver(); tryHydrate(); [250, 750, 1500, 3000, 6000, 10000].forEach(ms => window.setTimeout(tryHydrate, ms)); window.addEventListener('wnmu:manual-program-overrides-refresh', () => { state.hydrated = false; tryHydrate(); }); }
