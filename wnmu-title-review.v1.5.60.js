(function () {
  'use strict';
  const VERSION = 'v1.5.60-title-review-save-feedback-file-export';
  ...
  function bindInside(root) {
    ...
    root.querySelectorAll('.title-review-save-inline').forEach(btn => btn.addEventListener('click', () => {
      ...
      if (saveCorrection(from, to)) {
        markRowSaved(editor, from, to);
        setStatusMessage(`Saved correction: ${from} → ${to}. Saved in this browser${autoDownloadEnabled() ? ' and downloaded as a correction file' : '; use Download correction file for a reusable backup'}.`, 'good');
        if (autoDownloadEnabled()) downloadCorrectionFile();
      }
    }));
