(function () {
  'use strict';
  const VERSION = 'v1.5.28-explicit-commit-freeze-guard';
    ...
    const obs = new MutationObserver(muts => {
      for (const mut of muts) {
        ...
        for (const node of mut.addedNodes || []) {
          if (node.nodeType === 1 && (node.matches?.('#wnmuCellMenu') || node.querySelector?.('#wnmuCellMenu'))) { run(); return; }
        }
      }
    });
    obs.observe(document.documentElement, { childList: true, subtree: true });
    document.addEventListener('contextmenu', () => window.setTimeout(run, 40), true);
    document.addEventListener('click', event => { if (event.target?.closest?.('.program-cell')) window.setTimeout(run, 40); }, true);
  }
