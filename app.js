...
function renderSummary(schedule, verification) {
  ...
  document.getElementById('coverageFlag').textContent = verification.checks.everyDayHas48CoveredSlots
    ? '31/31 days at 48/48'
    : 'coverage issue';
  document.getElementById('daysFlag').textContent = `${verification.checks.actualDayCount} May dates`;
...
function renderMonthCards(schedule, verification, onSelect) {
  ...
  const tmpl = document.getElementById('dayCardTemplate');
...
function renderDayDetail(day) {
  document.getElementById('detailTitle').textContent = fmtDayTitle(day.date, day.day);
  document.getElementById('detailMeta').textContent = `${day.entries.length} airings laid across 48 half-hour slots.`;
  ...
  const tmpl = document.getElementById('airingTemplate');
...
function renderWeekGrids(schedule, onSelect) {
  const container = document.getElementById('weekGrids');
...
loadData().then(({ schedule, verification }) => {
  ...
  renderDayDetail(schedule.days[0]);
}).catch(err => {
