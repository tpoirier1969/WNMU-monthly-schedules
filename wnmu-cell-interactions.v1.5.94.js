(function () {
  ...
  const VERSION = 'v1.5.94-menu-entry-fast-path';
  ...
  function preventManagedMenuSubmit(event) {
    ...
    if (menu && !event.defaultPrevented) return false;
    if (menu) savePlanningTagsOnlyFromMenu(menu);
    return false;
  }
  ...
  function hardenMenuForm(menu) {
    ...
    forms.forEach(form => {
      ...
      form.addEventListener('keydown', event => {
        ...
        if (liveMenu) savePlanningTagsOnlyFromMenu(liveMenu);
      }, true);
  ...
  function openCellMenu(target, event) {
    ...
    if (target.entryId) menu.dataset.wnmuEntryId = target.entryId;
    else delete menu.dataset.wnmuEntryId;
