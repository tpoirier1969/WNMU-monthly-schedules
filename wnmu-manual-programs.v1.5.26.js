(function () {
  'use strict';
  const VERSION = 'v1.5.26-committed-table-manual-programs-only';
  ...
  function committedRows() {
    ...
    return rows.filter(row => row && row.is_active !== false && String(row.title_text || row.text || '').trim()).map(row => {
      const date = row.date || '';
  ...
  function installObserver() { if (observerInstalled) return; observerInstalled = true; const obs = new MutationObserver(muts => { let hit = false; for (const mut of muts) { if (mut.target?.id === 'wnmuCellMenu' || mut.target?.closest?.('#wnmuCellMenu')) hit = true; for (const node of mut.addedNodes || []) { if (node.nodeType === 1 && (node.matches?.('table.screen-week-grid,table.print-week-grid,#wnmuCellMenu') || node.querySelector?.('table.screen-week-grid,table.print-week-grid,#wnmuCellMenu'))) hit = true; } } if (hit) queueRefresh(30, true); }); obs.observe(document.documentElement, { childList: true, subtree: true, characterData: true }); }
