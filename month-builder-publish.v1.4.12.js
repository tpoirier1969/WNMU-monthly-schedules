...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.12-duration-token-parser';
  const state = { parsed: {} };
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    for (let offset = 0; offset < totalDays; offset += 1) {
      ...
      const normalizedEntries = deduped.map((entry, idx) => {
        ...
        // The ProTrack report duration values are durations, not start/end pairs.
        // Trust them when present, but do not let them run over the next parsed
        // program. If no duration token exists, default to 30 minutes so possible
        // holes remain visible instead of being swallowed.
        const durationMin = explicitLooksUsable ? Math.min(explicitDuration, maxUntilNextProgram) : 30;
        ...
        const hiddenHoleRisk = !explicitLooksUsable && maxUntilNextProgram > 30;
        return { date: dateKey, day: dayName, dayName, time: entry.time, title: entry.title, episode: entry.episode, seasonStart: inferSeasonStart(entry.episode), endTime, durationMin: slotCount * 30, slotCount, sourceDurationMin: entry.sourceDurationMin || null, sourceDurationToken: entry.sourceDurationToken || '', durationSource: explicitLooksUsable ? 'explicit-pdf-duration-token' : 'conservative-30min-default', hiddenHoleRisk };
      });
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.12 duration-token parser', days, weeks };
  ...
  function collectCoverageDiagnostics(schedule, verification) {
    ...
    (schedule.days || []).forEach(day => {
      (day.entries || []).forEach(entry => {
        if (entry.hiddenHoleRisk) {
          hiddenRisks.push({
            date: day.date,
            time: entry.time,
            title: entry.title,
            episode: entry.episode || '',
            reason: 'No PDF duration token was available; left following time visible as hole instead of stretching program.'
          });
        }
      });
    });
    return { exceptions, hiddenRisks };
  }
  ...
  function publishabilityFor(parsed) {
    ...
    if (diagnostics.hiddenRisks.length) fatal.push(`${diagnostics.hiddenRisks.length} program(s) using 30-minute default because no duration token was parsed`);
