(function () {
  const VERSION = 'v1.4.33-blank-note-no-duplicate-text';
  ...
  function renderBlankOverlays() {
    ...
    // Make sure blank cells are annotated before trying to attach note boxes.
    ...
    Object.entries(marks).forEach(([key, mark]) => {
      ...
      // One more fallback pass in case the table was rendered after the first annotation.
      if (!cell) {
        document.querySelectorAll('table.screen-week-grid').forEach(annotateTable);
      ...
      const height = Math.max(22, cellRect.height * slots - 4);
      const width = Math.max(46, cellRect.width - 8);

      // v1.4.32: draw fixed against the viewport. This avoids wrapper/offsetParent
      // weirdness and makes the box visible even if table wrappers are not positioned
      // the way the earlier overlay expected.
  ...
  function injectStyles() {
    if (document.getElementById('wnmuBlankSlotMenuV1433Styles')) return;
    const style = document.createElement('style');
    style.id = 'wnmuBlankSlotMenuV1433Styles';
    style.textContent = `
  ...
  function markVersion() {
    ...
    if (!flag) return;
    if (!flag.textContent.includes('blank note no duplicate 1.4.33')) {
      flag.textContent = `${flag.textContent} • blank note no duplicate 1.4.33`;
    }
  }
