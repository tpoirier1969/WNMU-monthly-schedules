(function () {
  ...
  const VERSION = 'v1.5.10-stable-box-size-target-cleanup';
  ...
  function writeCanonicalRecord(target, note) {
    ...
    if (mainKey) {
      ...
      if (target.entryId) {
        ...
        main[target.entryId] = { ...existing, tags: cleanTags, rectNote: record.cleared ? null : { x: 4, y: 4, w: 92, h: 92, text: record.text, durationMin: duration, anchor: 'left', cellOverrideV15: true } };
      }
  ...
  function tagPillHtml(tag) { return `<span class="tag-pill" style="--tag-color:${esc(tagMeta(tag).color || '#ddd')}">${esc(tagMeta(tag).label || tag)}</span>`; }
  function applyCellTagVisual(target, tags) {
    ...
    if (content) {
      ...
      if (active.length) {
        ...
        wrap.innerHTML = active.slice(0, 6).map(tagPillHtml).join('');
  ...
  function ensureOverlayLayer() {
    ...
    // Boxes are positioned against this layer's actual viewport rectangle,
    // not against an assumed document origin. This fixes left/down drift.
    return layer;
  }
  ...
  function saveFromCellMenu() {
    ...
    try {
      ...
      ensureOverlayLayer().querySelectorAll('.wnmu-cell-override-box').forEach(box => {
        if (box.dataset.wnmuDate === target.date && box.dataset.wnmuTime === target.time) box.remove();
      });
      paintOverlay(target, { text, durationMin: duration, tags });
      applyCellTagVisual(target, tags);
  ...
  function clearFromCellMenu() {
    ...
    selectedTarget = target;
    writeCanonicalRecord(target, { text: '', durationMin: 30, tags: explicitFalseTags(), cleared: true });
  ...
  function injectStyles() {
    if (document.getElementById('wnmuCellInteractionsV1510Styles')) return;
    const style = document.createElement('style'); style.id = 'wnmuCellInteractionsV1510Styles';
    style.textContent = `td.program-cell[data-wnmu-actionable="false"]{background:#fff!important}.program-cell[data-wnmu-actionable="false"] .program-tags,.program-cell[data-wnmu-actionable="false"]>.draw-rect-note,.program-cell[data-wnmu-actionable="false"]>.wnmu-blank-slot-content{display:none!important}#wnmuCellOverrideLayer{position:absolute;left:0;top:0;width:100%;min-height:100%;z-index:2147481000;pointer-events:none}.wnmu-cell-override-box{position:absolute;box-sizing:border-box;background:rgba(255,255,255,.96);border:2px solid rgba(12,18,32,.86);color:#111;border-radius:4px;box-shadow:0 2px 7px rgba(0,0,0,.22);padding:5px 6px 16px;overflow:hidden;font:12px/1.22 system-ui,sans-serif;pointer-events:auto}.wnmu-cell-override-text{white-space:normal;overflow:hidden;font-weight:700}.wnmu-cell-override-tags{display:flex;flex-wrap:wrap;gap:3px;margin-bottom:3px}.wnmu-cell-override-tags span{display:inline-flex;align-items:center;border:1px solid rgba(0,0,0,.25);border-radius:999px;padding:1px 5px;font-size:9px;font-weight:800;background:rgba(255,255,255,.72)}.wnmu-cell-override-box{cursor:context-menu}.wnmu-cell-override-duration{position:absolute;right:4px;bottom:2px;font-size:9px;opacity:.62}.program-cell.wnmu-has-cell-override>.draw-rect-note,.program-cell.wnmu-has-cell-override>.wnmu-blank-slot-content{display:none!important}#contextMenu{display:none!important;z-index:1}#wnmuCellMenu.wnmu-cell-menu{position:fixed;z-index:2147483646!important;min-width:420px;max-width:500px;max-height:calc(100vh - 28px);overflow:auto}.wnmu-cell-form{display:block}.wnmu-cell-tag-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin:10px 0}.wnmu-cell-tag-grid .check-row{min-height:39px;margin:0;box-sizing:border-box;display:flex;align-items:center;gap:8px;border-radius:9px}.wnmu-cell-note-tools{border:2px solid rgba(255,255,255,.32);border-radius:10px;padding:11px;margin:10px 0;background:rgba(255,255,255,.08)}.wnmu-cell-note-tools legend{font-weight:800}.wnmu-cell-duration-options{display:flex;flex-wrap:wrap;gap:5px;margin:5px 0 8px}.wnmu-cell-pill{display:inline-flex;align-items:center;gap:4px;font-size:12px;border:1px solid rgba(255,255,255,.3);border-radius:999px;padding:5px 8px;cursor:pointer;background:rgba(255,255,255,.08)}.wnmu-cell-pill input{margin:0}.manual-rect-text{width:100%;box-sizing:border-box;border-radius:8px;border:1px solid rgba(255,255,255,.28);padding:7px;resize:vertical;font:13px/1.3 system-ui,sans-serif;color:#111;background:#fff}.manual-rect-label{font-size:12px;font-weight:800;margin:6px 0 5px}.manual-rect-help{margin-top:6px;font-size:11px;opacity:.75;line-height:1.25}@media print{#wnmuCellOverrideLayer{display:none!important}}`;
    document.head.appendChild(style);
  }
  function relayoutVisibleOverlays() {
    const boxes = Array.from(ensureOverlayLayer().querySelectorAll('.wnmu-cell-override-box'));
    boxes.forEach(box => {
      const date = box.dataset.wnmuDate || '';
      const time = box.dataset.wnmuTime || '';
      if (!date || !time) return;
  ...
  function start() {
    ...
    window.addEventListener('resize', () => window.setTimeout(relayoutVisibleOverlays, 80), { passive: true });
    window.setTimeout(waitForReadyThenRestore, 450);
    window.setTimeout(sanitizeUneditableCells, 1400);
    window.setTimeout(relayoutVisibleOverlays, 2200);
    window.setTimeout(relayoutVisibleOverlays, 5200);
  }
