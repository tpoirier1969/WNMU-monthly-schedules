(function () {
  ...
  const JSON_CACHE_PREFIX = 'wnmu_json_cache_v1_3_0';
  ...
  function buildDateMap(schedule) {
    ...
    (schedule.weeks || []).forEach((week, weekIndex) => {
      week.forEach(day => {
        ...
        const sourceDay = (schedule.days || []).find(d => d.date === day.date);
        ...
        map[day.date] = { ...(sourceDay || day), entries: rawEntries, entryByTime };
      });
    });
  ...
  function renderFlags() {
    ...
    mustGet('versionFlag').textContent = `${BUILD_VERSION} • progressive • ${sourceBit}`;
