(function () {
  'use strict';
  const VERSION = 'v1.5.42-sticky-week-panel-header-block';
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      body.wnmu-fixed-framework-active > main.page {
        --wnmu-panel-head-height: 72px;
        --wnmu-week-heading-height: 42px;
      ...
      /* Keep the "Week grids" title/instructions stationary at the top of the schedule scroller. */
      body.wnmu-fixed-framework-active .weeks-panel > .panel-head {
        ...
        box-shadow: 0 1px 0 rgba(17,17,17,.18) !important;
      }
      ...
      body.wnmu-fixed-framework-active .week-grid-wrap {
        ...
        scroll-margin-top: calc(var(--wnmu-panel-head-height, 72px) + var(--wnmu-week-heading-height, 42px) + 12px);
      }

      /* Keep each current week label attached to the sticky schedule header block. */
      body.wnmu-fixed-framework-active .week-grid-wrap > h3 {
        position: sticky !important;
        top: var(--wnmu-panel-head-height, 72px) !important;
        ...
        border-top: 1px solid #c8d3e2 !important;
      ...
      /* The day/date header attaches under the sticky week title. */
      body.wnmu-fixed-framework-active table.screen-week-grid thead {
        position: sticky !important;
        top: calc(var(--wnmu-panel-head-height, 72px) + var(--wnmu-week-heading-height, 42px)) !important;
  ...
  function measureStickyBlock() {
    ...
    window.WNMU_STICKY_HEADER_BLOCK_METRICS = { version: VERSION, panelHeight, weekHeight, at: new Date().toISOString() };
  }

  function installMeasurementHooks() {
    measureStickyBlock();
    window.setTimeout(measureStickyBlock, 300);
    window.setTimeout(measureStickyBlock, 900);
    window.setTimeout(measureStickyBlock, 1800);
    window.addEventListener('resize', measureStickyBlock, { passive: true });
    const target = document.getElementById('weekGrids');
    if (target && 'MutationObserver' in window) {
      const observer = new MutationObserver(() => window.requestAnimationFrame(measureStickyBlock));
