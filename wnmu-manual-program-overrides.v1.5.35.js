(function () {
  'use strict';
  const VERSION = 'v1.5.35-explicit-commit-surgical-cleanup-menu-fix';
  ...
  const state = { readyKey: '', hydrated: false, tableAvailable: false, loadError: '', menuObserverInstalled: false, committedRows: [], storageGuardInstalled: false, scrubbing: false };
  ...
  async function loadTableOverrides() {
    ...
    clearOverlayDomForRows(clean);
    scheduleCommittedScrub('table-load');
  ...
  async function clearSharedStagingRange(row) {
    ...
    await deleteSharedMarkKeys(keys);
  ...
  function rowTitle(row) { return String(row && (row.title_text || row.text) || '').trim(); }
  ...
  function recordTouchesSlots(key, raw, slotSet) {
    ...
    return slotSet.has(`${date}__${time}`);
  }
  function shouldDeleteStagingRecord(raw) { if (!raw || typeof raw !== 'object') return false; const rect = raw.rectNote || raw.manualProgram || raw.manualProgramEntry || raw.programEntry || null; return !!(raw.cellOverrideV15 || raw.whiteout || raw.text || raw.note || raw.title || raw.title_text || raw.manualProgramOverrideV1524 || raw.manualProgramOverrideV1526 || (rect && (rect.text || rect.title || rect.title_text))); }
  ...
  async function commitFromMenu(menu) {
    ...
    try {
      ...
      menuStatus(menu, 'Committed permanently. Matching staged overlay data for this time block was deleted.');
    } catch (err) {
      menuStatus(menu, err.message || String(err), true);
  ...
  function prefillCommitted(menu) {
    ...
    menuStatus(menu, 'Committed program loaded. Change tags or text, then Commit to Schedule to update.');
    repositionMenu(menu);
  }
  ...
  function start() { ensureCommitMenuStyles(); installStorageGuard(); installMenuObserver(); tryHydrate(); [250, 750, 1500, 3000, 6000, 10000].forEach(ms => window.setTimeout(tryHydrate, ms)); [1200, 3500, 7500, 12000].forEach(ms => window.setTimeout(() => scrubCommittedStaging('startup-scrub', true), ms)); window.addEventListener('wnmu:manual-program-overrides-refresh', () => { state.hydrated = false; tryHydrate(); }); }
