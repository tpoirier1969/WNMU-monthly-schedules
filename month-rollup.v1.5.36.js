(function () {
  'use strict';
  const VERSION = 'v1.5.36-standalone-month-rollup';
  ...
  const TABLES = {
    ...
    manualOverrides: 'wnmu_monthly_schedule_overrides'
  };
  ...
  async function loadManualOverrides() {
    try {
      const select = 'entry_key,date,slot_time,duration_min,title_text,tags_json,is_active,updated_at';
      const rows = await restSelect(`/rest/v1/${TABLES.manualOverrides}?select=${select}&channel_code=eq.${encodeURIComponent(channel)}&month_key=eq.${encodeURIComponent(month)}&is_active=eq.true&order=date.asc&order=slot_time.asc&limit=2000`);
      return Array.isArray(rows) ? rows.filter(row => row.title_text) : [];
    } catch (err) {
      console.warn('Manual program overrides read skipped.', err);
      return [];
    }
  }
  ...
  function render(row, rows, sourceInfo) {
    ...
    $('scheduleLink').href = `${CONFIG.page}?month=${encodeURIComponent(month)}&v=1.5.36`;
    $('homeLink').href = 'index.html?v=1.5.36';
