(function () {
  'use strict';
  const VERSION = 'v1.5.54-sales-view-pledge-schedule-only';
  ...
  let pledgeScheduleCodeSet = new Set();
  ...
  function collectPledgeScheduleProgramKeys(value, depth = 0) {
    ...
    const codeKeys = [
      'nola_code', 'nolaCode', 'nola', 'program_code', 'programCode',
      'code', 'program_id', 'programId'
    ];

    const looksLikeScheduledProgram = Object.keys(value).some(key =>
      /program|nola|placement|airing|slot|title/i.test(key)
    );
    ...
    if (looksLikeScheduledProgram) {
      ...
      for (const key of codeKeys) {
        ...
        if (typeof raw === 'string' || typeof raw === 'number') {
          const codeKey = norm(String(raw));
          if (codeKey && codeKey.length > 2) pledgeScheduleCodeSet.add(codeKey);
        }
      }
    }
  ...
  async function loadOptionalPledgeLookup() {
    ...
    // Sales View should only treat programs as pledge/fundraising when they are
    // present in the actual Pledge Schedule data, not merely because they exist
    // somewhere in the larger pledge program library/catalog.
    ...
    window.WNMU_SALES_PLEDGE_LOOKUP = {
      ...
      codeCount: pledgeScheduleCodeSet.size,
  ...
  function isPledgeProgram(entry) {
    ...
    if (titleKey && pledgeScheduleTitleSet.has(titleKey)) return true;

    const codeKey = norm(entryCodeText(entry));
    if (codeKey) {
      for (const code of pledgeScheduleCodeSet) {
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.54` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.54`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.54 note: Fundraiser/Pledge detection now uses only actual Pledge Schedule data (pledge_fundraiser_schedules). The full pledge program library/catalog is no longer used as a fundraiser detector.</small></div>`;
    }
  }
