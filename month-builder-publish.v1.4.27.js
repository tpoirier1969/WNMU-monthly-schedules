...
(function () {
  ...
  const BUILD_VERSION = 'v1.4.27-weekday-header-anchor-episode-row-guard';
  const state = { parsed: {} };
  ...
  async function extractPdfPages(file) {
    ...
    for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
      ...
      const usePositioned = positionedRows >= eolRows && positionedRows > 0;
      const lines = usePositioned ? positionedLines : eolLines;
      pages.push({ pageNumber, lines, eolTimeRows: eolRows, positionedTimeRows: positionedRows, extractionMode: usePositioned ? 'positioned' : 'text-flow', columnSource: positioned.columnSource || 'unknown' });
    }
    return pages;
  }
  ...
  function buildMonthData(entries, monthKey, channelLabel) {
    ...
    const schedule = { version: `${channelLabel} ${monthKey} imported`, channel: channelLabel, month: monthKey, generatedAt: new Date().toISOString(), sourcePolicy: 'PDF import via month builder v1.4.27 weekday-header-anchored non-strict recovery parser with episode retention and service-code guard', days, weeks };
