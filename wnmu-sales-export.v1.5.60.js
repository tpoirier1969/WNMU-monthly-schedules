(function () {
  'use strict';
  const VERSION = 'v1.5.60-title-review-corrections';
  ...
  function collectPledgeScheduleProgramKeys(value, depth = 0) {
    ...
    if (looksLikeScheduledProgram) {
      ...
      // v1.5.60: intentionally do not collect code/program_id values here.
    }
  ...
  function bindTitleAuditControls() {
    // v1.5.60: Title cleanup is handled on the main 13.1 / 13.3 schedule pages, not in the Sales View.
  }
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.60` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.60`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.60 note: title cleanup is display-only; fundraiser detection uses exact title matches from actual Pledge Schedule data only.</small></div>`;
    }
  }
