(function () {
  ...
  const VERSION = 'v1.5.78-visible-custom-planning-label';
  ...
  function writeCanonicalRecord(target, note) {
    ...
    const otherLabel = cleanTags[OTHER_KEY] ? cleanOtherLabel(note.otherLabel || note.planningOtherLabel || note.customPlanningLabel || note.customTagLabel || note.otherText || '') : '';
    ...
    if (mainKey) {
      ...
      if (target.entryId) {
        ...
        main[target.entryId] = { ...existing, tags: cleanTags, otherLabel, planningOtherLabel: otherLabel, customPlanningLabel: otherLabel, customTagLabel: otherLabel, rectNote: record.cleared ? null : { x: 4, y: 4, w: 92, h: 92, text: record.text, durationMin: duration, anchor: 'left', cellOverrideV15: true } };
      }
  ...
  function legacyProgramNote(target) {
    ...
    for (const entryId of ids) {
      ...
      return { text: rect.text || item.text || '', durationMin: nearestDuration(extra.durationMin || rect.durationMin || Math.max(1, Number(target.cell?.rowSpan || 1)) * 30), tags: item.tags || {}, otherLabel: cleanOtherLabel(item.otherLabel || item.planningOtherLabel || item.customPlanningLabel || item.customTagLabel || item.otherText || ''), legacy: 'program', legacyEntryId: entryId };
    }
    return null;
  }
  ...
  function readEffectiveNote(target) {
    ...
    if (canon) return canon.cleared ? null : { text: canon.text || '', durationMin: nearestDuration(canon.durationMin || 30), tags: canon.tags || {}, otherLabel: cleanOtherLabel(canon.otherLabel || canon.planningOtherLabel || canon.customPlanningLabel || canon.customTagLabel || canon.otherText || ''), planningOtherLabel: cleanOtherLabel(canon.otherLabel || canon.planningOtherLabel || canon.customPlanningLabel || canon.customTagLabel || canon.otherText || ''), canonical: true };
  ...
  function applyCellTagVisual(target, tags, otherLabel = '') {
    ...
    if (otherText) cell.dataset.wnmuOtherLabel = otherText;
  ...
  function paintOverlay(target, note) {
    ...
    const noteOtherLabel = cleanOtherLabel(note.otherLabel || note.planningOtherLabel || note.customPlanningLabel || note.customTagLabel || note.otherText || '');
  ...
  function openCellMenu(target, event) {
    ...
    updateOtherMenuLabel(menu, note?.otherLabel || note?.planningOtherLabel || note?.customPlanningLabel || note?.customTagLabel || note?.otherText || '');
