...
const DATA_FILES = {
  schedule: 'schedule-data.v1.0.5.json',
  verification: 'verification.v1.0.5.json'
};
const STORAGE_KEY = 'wnmu1hdMay2026Marks.v1.0.5';
...
function getTagsForEntry(entry) {
  ...
  for (const key of TAG_ORDER) {
    out[key] = typeof saved[key] === 'boolean' ? saved[key] : defaults[key];
  }
  return out;
}
...
function createRollupLine(entry, includeFullMeta = false) {
  ...
  if (includeFullMeta) {
    ...
    line.innerHTML = `<strong>${escapeHtml(entry.title)}</strong>${episodePart ? ` <span class="meta">${escapeHtml(episodePart)}</span>` : ''}<br><span class="meta">${fmtTime(entry.time)} • ${escapeHtml(active)}</span>`;
  }
  return line;
}
...
function saveCurrentMenuMarks(clearAll = false) {
  ...
  const stored = {};
  for (const key of TAG_ORDER) {
    const current = clearAll ? false : !!form.elements[key].checked;
    if (current !== defaults[key]) stored[key] = current;
  }
  appState.marks[appState.currentEntryId] = stored;
  saveMarks();
  renderWeekGrids();
  renderMonthRollup();
  closeContextMenu();
