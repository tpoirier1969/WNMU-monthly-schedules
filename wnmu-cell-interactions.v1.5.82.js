(function () {
  ...
  const VERSION = 'v1.5.82-visible-custom-planning-label';
  ...
  function tagOrderBase() {
    const source = Array.isArray(cfg().tagOrder) ? cfg().tagOrder.slice() : Object.keys(TAG_FALLBACK).filter(k => k !== SATELLITE_KEY);
    let list = source.slice();
    if (channelCode() === '13.3') list = list.filter(tag => tag !== 'michigan');
    if (channelCode() === '13.1') list = list.filter(tag => tag !== 'arts');
    // Keep the custom tag internal, but do not require it to be exposed in page config.
    // The older shared renderer treats tagMeta.other as the literal label "Custom";
    // this module renders the actual typed text instead.
  ...
  function writeCanonicalRecord(target, note) {
    ...
    let cleanTags = normalizeTagsForStorage(note.tags || {});
    ...
    if (otherLabel) cleanTags[OTHER_KEY] = true;
  ...
  function openCellMenu(target, event) {
    ...
    updateOtherMenuLabel(menu, note?.otherLabel || note?.planningOtherLabel || note?.customPlanningLabel || note?.customTagLabel || note?.otherText || '', { force: true });
    ...
    if (customInput) {
      customInput.addEventListener('input', () => {
        ...
        if (liveTarget?.cell) forceCustomTagDisplayForCell(liveTarget.cell, liveLabel);
      });
    }
