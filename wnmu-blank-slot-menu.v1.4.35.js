(function () {
  const VERSION = 'v1.4.35-blank-box-save-and-inline-fix';
  ...
  function renderInlineBlankBox(cell, mark) {
    ...
    const cellHeight = Math.max(18, cell.getBoundingClientRect().height || cell.offsetHeight || 24);
    const height = Math.max(22, cellHeight * slots - 4);

    const box = document.createElement('div');
    box.className = 'wnmu-blank-note-inline';
    box.dataset.blankNoteInline = 'true';
  ...
  function renderBlankOverlays() {
    // v1.4.34: blank-slot note boxes are rendered directly inside the blank
    // cell by paintBlankCell(). The old floating overlay was too dependent on
    // wrapper/viewport positioning and could fail invisibly.
    document.querySelectorAll('.wnmu-blank-note-overlay').forEach(el => el.remove());
    document.querySelectorAll('.wnmu-blank-slot-cell').forEach(cell => {
      const date = cell.dataset.blankDate;
      const time = cell.dataset.blankTime;
      if (!date || !time) return;
      renderInlineBlankBox(cell, readMarkFor(date, time));
    });
  }
  ...
  function injectStyles() {
    if (document.getElementById('wnmuBlankSlotMenuV1435Styles')) return;
    const style = document.createElement('style');
    style.id = 'wnmuBlankSlotMenuV1435Styles';
    style.textContent = `
  ...
  function markVersion() {
    ...
    if (!flag) return;
    if (!flag.textContent.includes('blank box save fix 1.4.35')) {
      flag.textContent = `${flag.textContent} • blank box save fix 1.4.35`;
    }
  }
