(function () {
  ...
  const VERSION = 'v1.5.108-deferred-menu-save-custom-tag-isolation';
  ...
  function isManagedMenuForm(form) {
    return !!(form && (form.id === 'wnmuCellForm' || form.id === 'contextMenuForm' || form.closest?.('#wnmuCellMenu,#contextMenu')));
  }

  function preventManagedMenuSubmit(event) {
    const form = event.target?.closest?.('form') || event.target;
    if (!isManagedMenuForm(form)) return;
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation?.();
    const menu = form.closest?.('#wnmuCellMenu');
    if (menu) markMenuPlanningDirty(menu);
    return false;
  }

  function hardenMenuForm(menu) {
    const forms = [];
    const current = menu?.querySelector?.('#wnmuCellForm');
    const old = document.getElementById('contextMenuForm');
    if (current) forms.push(current);
    if (old) forms.push(old);
    forms.forEach(form => {
      if (!form || form.dataset.wnmuNoSubmitGuard === '1') return;
      form.dataset.wnmuNoSubmitGuard = '1';
      form.setAttribute('onsubmit', 'return false;');
      form.querySelectorAll('button:not([type])').forEach(button => button.setAttribute('type', 'button'));
      form.addEventListener('submit', preventManagedMenuSubmit, true);
      form.addEventListener('keydown', event => {
        ...
        event.stopPropagation();
        event.stopImmediatePropagation?.();
        const liveMenu = form.closest?.('#wnmuCellMenu');
        if (liveMenu) markMenuPlanningDirty(liveMenu);
      }, true);
  ...
  function installGlobalNoSubmitGuard() {
    if (window.WNMU_CELL_MENU_NO_SUBMIT_GUARD === VERSION) return;
    window.WNMU_CELL_MENU_NO_SUBMIT_GUARD = VERSION;
    ...
    document.addEventListener('click', event => {
      const button = event.target?.closest?.('#wnmuCellMenu button,#contextMenu button');
      if (!button) return;
  ...
  function cellMenuHtml(target, note) {
    ...
    return `<div class="context-menu-head"><div><h3>Schedule cell note</h3><div class="context-menu-meta">${esc(typeLabel)} • ${esc(target.date)} • ${esc(target.time)} • ${esc(noteSource)}</div></div><button type="button" class="menu-close" id="wnmuCellCloseBtn" aria-label="Close">×</button></div><form class="wnmu-cell-form" id="wnmuCellForm"><div class="wnmu-cell-tag-block"><div class="manual-rect-label">Planning tags</div><div class="wnmu-cell-tag-grid">${tagRows}</div></div><div class="wnmu-cell-custom-block"><label class="manual-rect-label" for="wnmuCellCustomTagText">Custom planning tag</label><div class="wnmu-custom-tag-row"><input id="wnmuCellCustomTagText" data-wnmu-custom-tag-input="true" class="manual-rect-text wnmu-custom-tag-input" type="text" maxlength="64" placeholder="Type custom tag…" value="${esc(otherLabel)}"><button type="button" class="btn ghost" id="wnmuCustomTagClearBtn">Clear custom tag</button></div><div class="manual-rect-help">This custom label shows as its own planning tag on the grid. Clear the field to remove it.</div></div><fieldset class="wnmu-cell-note-tools"><legend>White override box</legend><label class="manual-rect-label" for="wnmuCellText">Box note text</label><textarea id="wnmuCellText" class="manual-rect-text" rows="3" placeholder="Type what should show in this schedule slot">${esc(note?.text || '')}</textarea><div class="manual-rect-label wnmu-duration-label">Box size / length</div><div class="wnmu-cell-duration-options" role="radiogroup" aria-label="Box note length">${buildDurationRadios(defaultDuration, 'wnmuCellDuration')}</div><div class="manual-rect-help">Choose how many minutes the white box should cover. This controls the drawn box height.</div></fieldset><div class="rect-actions wnmu-unified-actions"><button type="button" class="btn ghost" id="wnmuCommitProgramBtn">Commit to Schedule</button><button type="button" class="btn ghost" id="wnmuCellSaveBtn">Save box note</button><button type="button" class="btn ghost" id="wnmuCellClearBtn">Clear box note</button><button type="button" class="btn ghost" id="wnmuRemoveCommittedProgramBtn" hidden>Remove committed program</button></div><div id="wnmuCellStatus" class="blank-save-status" aria-live="polite"></div></form>`;
  }
  ...
  function writeProgramPlanningTags(target, tags, otherLabel) {
    ...
    if (entryId) {
      ...
      if (existing.rectNote && typeof existing.rectNote === 'object') {
        next.rectNote = existing.rectNote;
      }
    ...
    // Blank/manual cells have no imported-program entryId. Keep planning tags in
    // the normal marks object, without manufacturing a canonical white-box record.
    // This keeps planning markup and White override box state independent.
    const all = readJson(key, {});
    const existing = all[target.key] && typeof all[target.key] === 'object' ? all[target.key] : {};
    const next = {
      ...existing,
      tags: cleanTags,
      otherLabel: cleanOther,
    ...
    const hasActiveTag = Object.values(cleanTags).some(Boolean);
    const hasExplicitFalse = Object.values(cleanTags).some(v => v === false);
    const hasNonPlanningContent = !!(next.rectNote || next.text || next.cellOverrideV15);
    if (!hasActiveTag && !hasExplicitFalse && !cleanOther && !hasNonPlanningContent) delete all[target.key];
    else all[target.key] = next;
    writeJson(key, all);
    setSatelliteForTarget(target, satelliteChecked);
    if (target.cell) {
      if (cleanOther) {
        target.cell.dataset.wnmuOtherLabel = cleanOther;
        target.cell.dataset.wnmuCustomPlanningTag = cleanOther;
      } else {
        delete target.cell.dataset.wnmuOtherLabel;
        delete target.cell.dataset.wnmuCustomPlanningTag;
      }
      applyCellTagVisual(target, cleanTags, cleanOther);
      forceCustomTagDisplayForCell(target.cell, cleanOther);
    }
    repaintSatelliteFeedsSoon(30);
    window.dispatchEvent(new CustomEvent('wnmu:cell-planning-tags-saved', { detail: { cellKey: target.key, version: VERSION } }));
    return true;
  }

  function menuPlanningSignature(menu) {
    const tags = readTagsFromMenu(menu);
    const otherLabel = readCustomOtherLabelFromMenu(menu);
    const normalized = {};
    allMenuTags().forEach(tag => { if (tag !== SATELLITE_KEY) normalized[tag] = !!tags[tag]; });
    normalized[OTHER_KEY] = !!otherLabel;
    return JSON.stringify({ tags: normalized, otherLabel });
  }
  function rememberMenuPlanningState(menu) {
    if (!menu) return '';
    const sig = menuPlanningSignature(menu);
    menu.dataset.wnmuPlanningSavedSig = sig;
    menu.dataset.wnmuPlanningDirty = 'false';
    return sig;
  }
  function markMenuPlanningDirty(menu) {
    if (!menu) return;
    const saved = menu.dataset.wnmuPlanningSavedSig || '';
    const current = menuPlanningSignature(menu);
    menu.dataset.wnmuPlanningDirty = String(saved !== current);
  }
  function savePlanningTagsOnlyFromMenu(menu) {
    ...
    window.WNMU_MENU_BULK_EDIT_ACTIVE_UNTIL = Date.now() + 1200;
    const ok = writeProgramPlanningTags(target, tags, otherLabel);
    if (ok && target?.cell) {
      if (otherLabel) {
        target.cell.dataset.wnmuOtherLabel = otherLabel;
        target.cell.dataset.wnmuCustomPlanningTag = otherLabel;
      } else {
        delete target.cell.dataset.wnmuOtherLabel;
        delete target.cell.dataset.wnmuCustomPlanningTag;
      }
    ...
    if (ok) rememberMenuPlanningState(menu);
    return ok;
  }
  function closeCellMenuAndPersist(options = {}) {
    const menu = document.getElementById('wnmuCellMenu');
    if (!menu || menu.classList.contains('hidden')) return;
    if (options.save !== false && menu.dataset.wnmuPlanningDirty === 'true') savePlanningTagsOnlyFromMenu(menu);
    hideCellMenu();
  }
  ...
  function openCellMenu(target, event) {
    ...
    menu.dataset.wnmuCellKey = target.key; menu.dataset.wnmuDate = target.date; menu.dataset.wnmuTime = target.time; menu.dataset.wnmuEntryId = target.entryId || '';
    menu.innerHTML = cellMenuHtml(target, note);
    hardenMenuForm(menu);
    const form = menu.querySelector('#wnmuCellForm');
    if (form) form.addEventListener('submit', event => {
      // Pressing Enter in the custom-tag text box should not submit the form and reload the page.
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation();
      return false;
    ...
    rememberMenuPlanningState(menu);
    ...
    if (customInput) {
      customInput.addEventListener('input', () => {
        ...
        updateOtherMenuLabel(menu, liveLabel);
        markMenuPlanningDirty(menu);
      });
      customInput.addEventListener('blur', () => markMenuPlanningDirty(menu));
    }
    ...
    menu.querySelector('.wnmu-cell-tag-grid')?.addEventListener('change', () => markMenuPlanningDirty(menu));
    menu.querySelector('#wnmuCellCloseBtn')?.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); closeCellMenuAndPersist(); });
    ...
    window.dispatchEvent(new CustomEvent('wnmu:cell-menu-opened', { detail: { version: VERSION, target: { key: target.key, date: target.date, time: target.time, entryId: target.entryId || '', sourceType: target.sourceType || '' } } }));
  ...
  function installHandlers() {
    ...
    document.addEventListener('click', event => { const menu = document.getElementById('wnmuCellMenu'); if (!menu || menu.classList.contains('hidden')) return; if (menu.contains(event.target)) return; closeCellMenuAndPersist(); }, true);
    document.addEventListener('keydown', event => { if (event.key === 'Escape') closeCellMenuAndPersist(); });
  }
  function start() {
    hideOldContextMenu();
    injectStyles();
    installHandlers();
    installGlobalNoSubmitGuard();
    hardenMenuForm(document.getElementById('wnmuCellMenu'));
