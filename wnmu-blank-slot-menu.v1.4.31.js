(function () {
  const VERSION = 'v1.4.31-blank-box-note-overlay-fix';
  ...
  function renderBlankOverlays() {
    ...
    Object.entries(marks).forEach(([key, mark]) => {
      ...
      const cell = cellsForBlankKey(key)[0];
      if (!cell) return;
      ...
      const left = cellRect.left + window.scrollX + 4;
      const top = cellRect.top + window.scrollY + 2;
  ...
  function injectStyles() {
    if (document.getElementById('wnmuBlankSlotMenuV1431Styles')) return;
    const style = document.createElement('style');
    style.id = 'wnmuBlankSlotMenuV1431Styles';
    style.textContent = `
  ...
  function markVersion() {
    ...
    if (!flag) return;
    if (!flag.textContent.includes('blank box overlay fix 1.4.31')) {
      flag.textContent = `${flag.textContent} • blank box overlay fix 1.4.31`;
    }
  }
