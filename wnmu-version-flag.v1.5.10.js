(function () {
  ...
  const PACKAGE_VERSION = 'v1.5.10';
  const PACKAGE_LABEL = 'component versions + sticky headers + target cleanup';
  const KNOWN_PARTS = [
    { key: 'package', label: 'Delivered package', version: `${PACKAGE_VERSION} ${PACKAGE_LABEL}` },
    { key: 'home', label: 'Home page shell', version: 'index.html v1.5.10' },
    { key: 'schedule-shell', label: 'Schedule page shell', version: 'index131/133 HTML v1.4.1 + package v1.5.10' },
    { key: 'builder-shell', label: 'Month Builder shell', version: 'month-builder HTML v1.4.1 / parser v1.4.16' },
    { key: 'interactions', label: 'Cell interactions / box notes', global: 'WNMU_CELL_INTERACTIONS_VERSION', fallback: 'v1.5.10 stable box-size target cleanup' },
  ...
  function pageShellVersion() {
    ...
    if (page === 'index.html' || !page.includes('.html')) return 'Home page shell v1.5.10';
    if (/index13[13]\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.10`;
    if (/month-builder\.v1\.4\.1\.html/i.test(page)) return `${page} shell v1.4.1 + package v1.5.10`;
    return `${page} + package v1.5.10`;
  }
  ...
  function componentRows() {
    ...
    document.querySelectorAll('script[src], link[rel="stylesheet"][href], link[rel~="icon"][href], link[rel="manifest"][href], link[rel="apple-touch-icon"][href]').forEach(el => {
      ...
      if (!/wnmu|app\.13|archive|current-loader|postload|satellite|checkbox|builder|version|config|styles|print|favicon|icon|manifest|pdf/i.test(raw)) return;
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .wnmu-component-versions{margin-top:8px;max-width:min(720px,100%);font:12px/1.35 system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#172033;z-index:2147482000;position:relative}
      .wnmu-component-versions summary{display:inline-flex;align-items:center;gap:6px;cursor:pointer;user-select:none;border:1px solid #c8d3e2;border-radius:999px;background:#fff;color:#17345f;font-weight:850;padding:6px 10px;box-shadow:0 1px 3px rgba(0,0,0,.08)}
      .wnmu-component-versions[open] summary{border-radius:12px 12px 0 0}
      ...
      .flagbar .wnmu-component-versions{display:inline-block;margin-left:8px;margin-top:0;vertical-align:middle}.flagbar .wnmu-component-versions .wnmu-component-box{position:absolute;right:0;min-width:min(720px,calc(100vw - 32px))}
