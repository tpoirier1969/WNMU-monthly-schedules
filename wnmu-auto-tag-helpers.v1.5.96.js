(function () {
  'use strict';
  const VERSION = 'v1.5.96-persistent-premiere-auto-tags';
  ...
  let lastAutoTagsByEntryId = new Map();
  let sanitizeTimer = 0;
  let menuTimer = 0;
  let sanitizeActive = false;
  let scheduleIndexCache = null;
  let scheduleIndexCacheKey = '';
  let suppressStorageSanitize = 0;
  let storeCache = { mainRaw: null, canonRaw: null, main: {}, canonical: {}, mainAliases: new Map(), canonicalAliases: new Map() };
  ...
  function makeCellKey(date, time){ const m = monthKey() || String(date || '').slice(0, 7); return `${channelCode()}__${m}__${date}__${time}`; }
  ...
  function shouldAutoSeasonStart(entry){
    if (!entry || !entry.seasonStart) return false;
    if (matchesAny(entry, cfg().suppressSeasonStartRules || [])) return false;
    // Older channel config calls these suppressNewSeriesRules, but they really
    // describe time windows where automatic premiere-style tags should stay off.
    if (matchesAny(entry, cfg().suppressNewSeriesRules || [])) return false;
    return true;
  }
  ...
  function addAlias(map, alias, record){
  ...
  function indexStoreAliases(store){
    const map = new Map();
    Object.values(store || {}).forEach(record => {
      if (!record || typeof record !== 'object') return;
      addAlias(map, record.sourceEntryId || record.source_entry_id, record);
      addAlias(map, record.legacyEntryId || record.legacy_entry_id, record);
      const rect = record.rectNote && typeof record.rectNote === 'object' ? record.rectNote : null;
      if (rect) {
        addAlias(map, rect.sourceEntryId || rect.source_entry_id, record);
        addAlias(map, rect.legacyEntryId || rect.legacy_entry_id, record);
      }
    });
    return map;
  }
  function cachedStores(){
    ...
    const canonKey = canonicalStoreKey();
    const mainRaw = mainKey ? (localStorage.getItem(mainKey) || '{}') : '{}';
    const canonRaw = canonKey ? (localStorage.getItem(canonKey) || '{}') : '{}';
    if (storeCache.mainRaw === mainRaw && storeCache.canonRaw === canonRaw) return storeCache;
    let main = {}, canonical = {};
    try { main = JSON.parse(mainRaw || '{}') || {}; } catch { main = {}; }
    try { canonical = JSON.parse(canonRaw || '{}') || {}; } catch { canonical = {}; }
    storeCache = { mainRaw, canonRaw, main, canonical, mainAliases: indexStoreAliases(main), canonicalAliases: indexStoreAliases(canonical) };
    return storeCache;
  }
  function invalidateStoreCache(){ storeCache.mainRaw = null; storeCache.canonRaw = null; }
  ...
  function allTagKeys(){ const list = Array.isArray(cfg().tagOrder) ? cfg().tagOrder.slice() : []; if (!list.includes('highlight')) list.unshift('highlight'); if (!list.includes('newSeason')) list.splice(Math.min(2, list.length), 0, 'newSeason'); if (!list.includes(OTHER_KEY)) list.splice(Math.min(5, list.length), 0, OTHER_KEY); return Array.from(new Set(list.filter(tag => tag !== SATELLITE_KEY))); }
  function tagOrder(){ return allTagKeys(); }
  function tagPriority(){ const base = Array.isArray(cfg().tagPriority) ? cfg().tagPriority.slice() : tagOrder(); return ['highlight','newSeries','newSeason', ...base.filter(tag => tag !== 'highlight' && tag !== 'newSeries' && tag !== 'newSeason')]; }
  function tagMeta(){ return cfg().tagMeta || {}; }
  function cleanOtherLabel(value){ return String(value || '').trim().replace(/\s+/g, ' ').slice(0, 64); }
  function tagLabel(tag, otherLabel = ''){ if (tag === OTHER_KEY) return cleanOtherLabel(otherLabel); return tagMeta()[tag]?.label || (tag === SATELLITE_KEY ? 'PBS Feed' : tag); }
  ...
  function customOtherLabelFromExistingPills(cell){
    ...
    known.add('Satellite Feed'); known.add('PBS Feed'); known.add('New Season'); known.add('Custom');
  ...
  function customOtherLabelForCell(cell){
    if (!cell) return '';
    let label = cleanOtherLabel(cell.dataset?.wnmuOtherLabel || cell.dataset?.wnmuCustomPlanningTag || '');
    if (label) return label;
    for (const record of recordsForCell(cell)) {
      label = otherLabelFromRecord(record);
      if (label) return label;
    }
    // Final fallback: if the current DOM already has a non-standard tag pill, keep it.
    // This prevents later grid sanitize passes from erasing a custom tag that was drawn
    // immediately while the user typed but before remote/local sync finished.
  ...
  function classifyAutoPremiereTag(entry, index){
    if (!entry || !entry.seasonStart || !shouldAutoNewSeries(entry)) return '';
    return isNewSeasonCandidate(entry, index) ? 'newSeason' : 'newSeries';
  }
  ...
  function defaultTagsForEntry(entry, id){
    ...
    const index = buildScheduleIndex(); const autoPremiereTag = classifyAutoPremiereTag(entry, index);
    if (autoPremiereTag === 'newSeason') {
      ...
      out.newSeries = false;
    } else if (autoPremiereTag === 'newSeries') {
      out.newSeries = disabledTags.newSeries === true ? false : true;
      out.newSeason = false;
    }
  ...
  function detectAutoPremiereEntries(){
    const index = buildScheduleIndex(); const out = [];
    index.entries.forEach(entry => {
      const tag = classifyAutoPremiereTag(entry, index);
      if (tag) out.push({ entry, tag });
    });
    return out;
  }
  ...
  function applyDerivedMarks(){
    ...
    const marks = readJson(key, {}); const disabled = readJson(disabledKey(), {});
    const candidates = detectAutoPremiereEntries(); const ids = new Set(); const tagById = new Map();
    const autoState = { version: VERSION, at: new Date().toISOString(), newSeriesEntryIds: [], newSeasonEntryIds: [] };
    candidates.forEach(({ entry, tag }) => {
      const id = entryId(entry); if (!id) return;
      ids.add(id); tagById.set(id, tag);
      if (tag === 'newSeason') autoState.newSeasonEntryIds.push(id);
      else if (tag === 'newSeries') autoState.newSeriesEntryIds.push(id);
      const existing = marks[id] || {}; const disabledTags = disabled[id] || {};
      if (disabledTags[tag] === true || (hasExplicitTag(existing, tag) && tagsObject(existing)[tag] === false)) return;
      let next = existing;
      if (tag === 'newSeason') {
      ...
      } else if (tag === 'newSeries') {
        if (!hasExplicitTag(existing, 'newSeries')) next = setTag(next, 'newSeries', true);
        if (!hasExplicitTag(existing, 'newSeason')) next = setTag(next, 'newSeason', false);
      }
      marks[id] = next;
    });
    lastAutoEntryIds = ids; lastAutoTagsByEntryId = tagById;
    writeJson(autoStateKey(), autoState); writeJson(key, marks);
    window.WNMU_AUTO_TAG_HELPERS_STATUS = {
      version: VERSION,
      newSeriesCount: autoState.newSeriesEntryIds.length,
      newSeasonCount: autoState.newSeasonEntryIds.length,
      storageKey: key,
    ...
    Storage.prototype.setItem = function patchedAutoTagSetItem(key, value){
      ...
      try {
        ...
        lastAutoEntryIds.forEach(id => {
          const autoTag = lastAutoTagsByEntryId.get(id) || 'newSeason';
          const beforeTags = tagsObject(before[id]); const afterTags = tagsObject(after[id]);
          if (beforeTags[autoTag] === true && afterTags[autoTag] !== true) {
            disabled[id] = { ...(disabled[id] || {}), [autoTag]: true, at: new Date().toISOString() };
            after[id] = setTag(after[id] || {}, autoTag, false); changed = true;
          }
        });
      ...
      if (!suppressStorageSanitize) scheduleSanitize(180);
      return result;
    };
  }
  ...
  function cellKeyForCell(cell){
    if (!cell) return '';
    if (cell.dataset?.wnmuCellKey) return cell.dataset.wnmuCellKey;
    const parsed = parseEntryId(cell.dataset?.entryId || '');
    if (parsed?.date && parsed?.time) return makeCellKey(parsed.date, parsed.time);
    if (cell.dataset?.wnmuDate && cell.dataset?.wnmuTime) return makeCellKey(cell.dataset.wnmuDate, cell.dataset.wnmuTime);
    return '';
  }

  function recordsForCell(cell){
    if (!storageKey() || !cell) return [];
    const stores = cachedStores();
    const main = stores.main || {};
    const canonical = stores.canonical || {};
    const entryId = cell.dataset?.entryId || '';
    const cellKey = cellKeyForCell(cell);
    const out = [];
    const push = (record) => { if (record && typeof record === 'object' && !out.includes(record)) out.push(record); };
    push(main[entryId]);
    push(main[cellKey]);
    push(canonical[cellKey]);
    if (entryId) {
      (stores.mainAliases.get(entryId) || []).forEach(push);
      (stores.canonicalAliases.get(entryId) || []).forEach(push);
    }
    return out;
  }

  function storedTagsForCell(cell){
    const records = recordsForCell(cell);
    const explicit = records.find(record => hasAnyExplicitStoredTag(tagsObject(record)));
    return explicit ? tagsObject(explicit) : {};
  }
  ...
  function effectiveTagsForCell(cell){
    ...
    const stored = storedTagsForCell(cell);
    if (hasAnyExplicitStoredTag(stored)) {
      const out = {};
      allTagKeys().forEach(tag => { out[tag] = stored[tag] === true; });
      if (otherLabel) out[OTHER_KEY] = true;
      return out;
    }
    const defaults = defaultTagsForEntryId(entryId);
    if (activeTagList(defaults, true).length || otherLabel) {
      const out = { ...(defaults || {}) };
      if (otherLabel) out[OTHER_KEY] = true;
      return out;
    }
    const fromLabels = tagsFromLabels(labelsForCell(cell));
    if (otherLabel) fromLabels[OTHER_KEY] = true;
    return fromLabels;
  }

  window.WNMU_AUTO_TAG_HELPERS_API = {
    version: VERSION,
    effectiveTagsForCell,
    sanitizeVisibleHighlights: () => sanitizeVisibleHighlights(),
    scheduleSanitize: (delay = 80) => scheduleSanitize(delay),
    customOtherLabelForCell,
  ...
  function renderTags(cell, tags){
    ...
    const otherLabel = customOtherLabelForCell(cell);
    // Custom labels are their own draw condition. Do not require the incoming tags
    // object to already contain other:true, because older saves and canonical-cell
    // records may hold the label while the tag map is incomplete.
  ...
  function writeExplicitTagsForMenu(menu, tags){
    ...
    suppressStorageSanitize += 1;
    try { writeJson(key, marks); } finally { suppressStorageSanitize = Math.max(0, suppressStorageSanitize - 1); }
    invalidateStoreCache();
  ...
  function installImmediateMenuSync(){
    ...
    document.addEventListener('change', event => {
      ...
      // The clicked cell was already redrawn by writeExplicitTagsForMenu; avoid a full-grid repaint on every checkbox change.
    }, true);
    ...
    document.addEventListener('input', event => {
      ...
      customTagWriteTimer = window.setTimeout(() => {
        const tags = readMenuTags(menu);
        writeExplicitTagsForMenu(menu, tags);
        // The target cell is already redrawn; leave full-grid sanitize for grid-load/remote-load events.
      }, 350);
    }, true);
    ...
    document.addEventListener('click', event => {
      ...
      if (!menu) return;
      if (!target.closest('#wnmuCommitProgramBtn, #wnmuCellSaveBtn, #wnmuCustomTagClearBtn')) return;
      window.setTimeout(() => { const tags = readMenuTags(menu); writeExplicitTagsForMenu(menu, tags); }, 30);
    }, true);
  }
  ...
  function installObservers(){
    ...
    if ('MutationObserver' in window) {
      const bodyObs = new MutationObserver(mutations => {
        if (mutations.some(m => Array.from(m.addedNodes || []).some(n => n && n.nodeType === 1 && (n.id === 'weekGrids' || n.querySelector?.('#weekGrids'))))) scheduleSanitize(260);
      });
      ...
      if (host) {
        ...
        const gridObs = new MutationObserver(mutations => {
          // Only react to actual grid replacement/content being added. Tag pills are our own
          // markup; reacting to them created repeated full-grid repair work during long sessions.
          if (mutations.some(m => Array.from(m.addedNodes || []).some(looksLikeGridContent))) scheduleSanitize(260);
        });
        gridObs.observe(host, { childList:true, subtree:true });
      }
    }
    [600,1600,3200].forEach(ms => window.setTimeout(() => { sanitizeVisibleHighlights(); scheduleCustomLabelRepair(40); }, ms));
  }
  ...
  function start(){ installStorageDisableHook(); installRendererAppendHook(); installObservers(); installRemoteStorageListener(); scheduleCustomLabelRepair(900); }
