(function () {
  'use strict';
  const VERSION = 'v1.5.65-title-review-no-saved-list';
  ...
  function cleanupEmptyGroups(start) {
    const variant = start?.closest?.('.title-review-variant');
    if (variant && !variant.querySelector('.title-review-row')) variant.remove();
    document.querySelectorAll('#titleReviewOutput details.title-review-group').forEach(group => {
      const hasRows = !!group.querySelector('.title-review-row, .title-review-variant');
      const summary = group.querySelector('summary')?.textContent || '';
      if (!hasRows) {
        const list = group.querySelector('.title-review-list');
        const msg = '<p class="title-review-muted title-review-empty-msg">All visible items in this group have been resolved.</p>';
        if (list) list.innerHTML = msg;
        else if (!group.querySelector('.title-review-empty-msg')) group.insertAdjacentHTML('beforeend', msg);
      }
    });
  }
  function resolveReviewRow(row, from, to) {
    if (!row) return;
    ...
    row.setAttribute('data-title-review-resolved', '1');
    if (row.classList.contains('saved')) {
      ...
      if (strong) strong.textContent = to;
  ...
  function markRowSaved(editor, from, to) {
    const row = editor?.closest?.('.title-review-row');
    if (!row) return;
    resolveReviewRow(row, from, to);
  }
  async function applyVariantGroup(button) {
    const variant = button?.closest?.('.title-review-variant');
    if (!variant) return;
    const to = cleanTitle(button.getAttribute('data-title-to') || variant.getAttribute('data-title-preferred') || '');
    if (!to) {
      setStatusMessage('Apply to all could not run because no preferred title was found.', 'bad');
      return;
    }

    // Only save rows that are actual alternate spellings. The preferred row is
    // often rendered inside the same group for context; leaving that preferred
    // row behind made the group stay on screen with the button stuck at
    // "Applying…" even after the real corrections had saved.
    const titleButtons = Array.from(variant.querySelectorAll('.title-review-row [data-title-from]'));
    const rows = titleButtons
      .map(btn => ({ btn, row: btn.closest('.title-review-row'), from: cleanTitle(btn.getAttribute('data-title-from') || '') }))
      .filter(item => item.row && item.from && item.from !== to);

    const originalLabel = button.textContent || 'Apply to all';
    button.disabled = true;
    button.textContent = 'Applying…';

    if (!rows.length) {
      variant.remove();
      cleanupEmptyGroups(document.getElementById('titleReviewOutput'));
      setStatusMessage(`No alternate spellings needed correction for ${to}.`, 'info');
      return;
    }
    ...
    try {
      for (const item of rows) {
        try {
          const saved = await saveCorrectionEverywhere(item.from, to);
          if (saved.localOk) {
            localCount += 1;
            if (saved.remoteOk) remoteCount += 1;
            resolveReviewRow(item.row, item.from, to);
          } else failedCount += 1;
        } catch (err) {
          failedCount += 1;
          console.error(`${VERSION}: apply-to-all save failed`, item.from, err);
        }
      }

      const remoteNote = remoteCount === localCount ? 'Supabase + browser' : 'browser fallback used for at least one item';
      setStatusMessage(`Applied “${to}” to ${localCount} title variant${localCount === 1 ? '' : 's'} (${remoteNote})${failedCount ? `; ${failedCount} failed` : ''}.`, failedCount ? 'info' : 'good');

      // If at least one correction saved, this suggestion group has been handled.
      // Remove the entire group, including the already-correct preferred row, so
      // the user is not left staring at a disabled Applying button.
      if (localCount > 0) {
        variant.remove();
      } else if (variant.isConnected) {
        button.disabled = false;
        button.textContent = originalLabel;
      }
    } finally {
      if (variant.isConnected && !variant.querySelector('.title-review-row')) variant.remove();
      if (variant.isConnected && button.isConnected && button.disabled) {
        button.disabled = false;
        button.textContent = failedCount ? 'Retry apply to all' : originalLabel;
      }
      cleanupEmptyGroups(document.getElementById('titleReviewOutput'));
    }
  }
    ...
    if (result) {
      ...
      if (result.variants.length) {
        ...
        result.variants.slice(0,120).forEach(group => {
          html += `<div class="title-review-variant" data-title-preferred="${esc(group.preferred)}"><div class="title-review-preferred"><span>Suggested preferred title: <strong>${esc(group.preferred)}</strong></span><button type="button" class="title-review-apply-all" data-title-to="${esc(group.preferred)}">Apply to all</button></div>`;
  ...
  function bindInside(root) {
    ...
    // v1.5.64: the inline editor is inserted after the scan list is rendered.
    ...
    root.addEventListener('click', async event => {
      const applyAllBtn = event.target.closest?.('.title-review-apply-all');
      if (applyAllBtn && root.contains(applyAllBtn)) {
        event.preventDefault();
        event.stopPropagation();
        await applyVariantGroup(applyAllBtn);
        return;
      }
