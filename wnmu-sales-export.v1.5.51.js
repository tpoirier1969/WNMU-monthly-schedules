(function () {
  'use strict';
  const VERSION = 'v1.5.51-sales-view-timeslot-rules';
  ...
  async function optionalRest(pathAndQuery) {
    ...
    catch (err) { console.info(`${VERSION}: optional lookup skipped`, err.message || err); return null; }
  }
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.51` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.51`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.51 note: the Sales View now loads config.js directly before the export script and waits for credentials before reading Supabase.</small></div>`;
    }
  }
