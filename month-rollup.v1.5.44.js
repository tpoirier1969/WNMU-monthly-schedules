(function () {
  'use strict';
  const VERSION = 'v1.5.44-standalone-month-rollup';
  ...
  const TABLES = {
    ...
    manualOverrides: 'wnmu_monthly_schedule_overrides' // legacy ignored in v1.5.44
  };
  ...
  const CHANNELS = {
    '13.1': {
      ...
      tagMeta: {
        newSeries: 'New Series', newSeason: 'New Season', highlight: 'Highlight', oneOff: 'One Off', monthlyTopic: 'Monthly topic', fundraiser: 'Fundraiser', programmersChoice: "Programmer's Choice", holiday: 'Holiday', noteworthy: 'Noteworthy', educational: 'Educational', local: 'Local', michigan: 'Michigan'
      }
    },
    '13.3': {
      label: 'WNMU3PL',
      ...
      tagMeta: {
        newSeries: 'New Series', newSeason: 'New Season', highlight: 'Highlight', oneOff: 'One Off', monthlyTopic: 'Monthly topic', fundraiser: 'Fundraiser', programmersChoice: "Programmer's Choice", holiday: 'Holiday', noteworthy: 'Noteworthy', educational: 'Educational', local: 'Local', arts: 'Arts'
      }
    }
  };
  ...
  async function loadManualOverrides() {
    // v1.5.44: manual commits are physical schedule_json entries.
    // Ignore legacy override-table rows so rollup cannot double-count or suppress imported rows.
    return [];
  }
  ...
  function buildRows(entries, marks, manualOverrides) {
    const usedMarkKeys = new Set();
    const rows = [];
    (manualOverrides || []).forEach(row => {
      const tagsObj = row.tags_json || {};
      const tags = activeLabels(CONFIG.tagOrder.reduce((acc, tag) => { acc[tag] = !!tagsObj[tag]; return acc; }, {}));
      rows.push({ type: 'manual', date: row.date, time: row.slot_time, durationMin: row.duration_min || 30, title: row.title_text || 'Manual program', episode: '', code: '', tags, note: '', sort: `${row.date}${row.slot_time}` });
      if (row.entry_key) usedMarkKeys.add(row.entry_key);
    });
    const manualCoveredSlots = new Set();
    (manualOverrides || []).forEach(row => {
      const start = timeToSlot(row.slot_time);
      const slots = Math.max(1, Math.round(Number(row.duration_min || 30) / 30));
      for (let i = 0; i < slots; i += 1) {
        const slot = start + i;
        manualCoveredSlots.add(`${row.date}__${String(Math.floor(slot / 2)).padStart(2, '0')}:${slot % 2 ? '30' : '00'}`);
      }
    });
    entries.forEach(entry => {
      if (manualCoveredSlots.has(`${entry.date}__${entry.time}`)) return;
  ...
  function render(row, rows, sourceInfo) {
    ...
    $('pageSub').textContent = `${rows.length} active item${rows.length === 1 ? '' : 's'} • ${sourceInfo}`;
    $('scheduleLink').href = `${CONFIG.page}?month=${encodeURIComponent(month)}&v=1.5.44`;
    $('homeLink').href = 'index.html?v=1.5.44';
    $('versionBadge').textContent = `Month Rollup ${VERSION}`;
    const body = $('rollupBody');
    if (!rows.length) {
      body.innerHTML = `<div class="empty">No active tagged programs or program replacements found for ${esc(CONFIG.label)} ${esc(monthLabel(month))}.</div>`;
      return;
    }
    document.body.classList.toggle('many-items', rows.length > 42);
    body.innerHTML = `
      <table class="rollup-table">
        <thead><tr><th>Date</th><th>Time</th><th>Min</th><th>Program / replacement</th><th>Active tags</th></tr></thead>
        <tbody>${rows.map(item => {
          const classes = [item.type === 'blank' ? 'blank-row' : '', item.type === 'manual' ? 'manual-row' : ''].filter(Boolean).join(' ');
          const meta = [item.episode, item.code].filter(Boolean).map(bit => `<div class="muted">${esc(bit)}</div>`).join('');
          const note = item.note ? `<div class="note-inline">${esc(item.note)}</div>` : '';
          return `
          <tr class="${classes}">
            <td>${esc(fmtDate(item.date))}</td>
            <td>${esc(fmtTime(item.time))}</td>
            <td>${esc(item.durationMin || '')}</td>
            <td><div class="program-main">${esc(item.title)}</div>${meta}${note}</td>
            <td><span class="tag-text">${esc((item.tags || []).join(', '))}</span></td>
          </tr>`;
        }).join('')}
        </tbody>
      </table>`;
  ...
  async function start() {
    try {
      ...
      const rows = buildRows(allEntries(schedule), marks, manualOverrides);
      render(row, rows, Object.keys(shared).length ? 'shared Supabase marks + local overlay' : 'local/default marks');
    } catch (err) {
      console.error(err);
      $('rollupBody').innerHTML = `<div class="error">${esc(err.message || err)}</div>`;
    }
  }
