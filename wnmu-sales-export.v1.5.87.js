(function () {
  'use strict';
  const VERSION = 'v1.5.87-sponsor-guide-visual-topic-runs';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      :root {
        ...
        --sales-kids:#fff0a8; --sales-health:#bfead2; --sales-financial:#e7c7f4; --sales-news:#bfd7f2;
        --sales-food:#f8c9a6; --sales-home:#d9ccb3; --sales-travel:#aee9e2; --sales-michigan:#b8e0f4; --sales-local:#c7e7b8;
        --sales-arts:#d2c2f0; --sales-environmental:#d7e6a7; --sales-science:#b9e6f3; --sales-history:#f1d0a6; --sales-documentary:#c9cfee;
        --sales-drama:#f0bfd9; --sales-talk:#c7d8ed; --sales-native:#e6bdaa; --sales-variety:#ece5a8; --sales-choice:#cde4ac; --sales-specials:#c8c5e6; --sales-fundraiser:#f3c1c8; --sales-program:#eceff4; --sales-blank:#ffffff;
      }
  ...
  function render(row) {
    ...
    const scheduleHref = channel === '13.3' ? `index133.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.87` : `index131.v1.4.1.html?month=${encodeURIComponent(month)}&v=1.5.87`;
  ...
  async function start() {
    ...
    try {
      ...
      const root = document.getElementById(ROOT_ID);
      if (root) root.innerHTML = `<div class="sales-error"><strong>Could not build Sales View.</strong><br>${esc(err.message || String(err))}<br><br><small>v1.5.86 note: sales category colors use an extra-light pastel target: a soft but more distinguishable sponsor-guide palette; fundraiser detection still uses only exact date/time placements from the actual Fundraising Schedule.</small></div>`;
    }
  }
