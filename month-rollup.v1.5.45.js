(function () {
  'use strict';
  const VERSION = 'v1.5.45-standalone-month-rollup';
  ...
  const TABLES = {
    ...
    manualOverrides: 'wnmu_monthly_schedule_overrides' // legacy ignored in v1.5.45
  };
  ...
  async function loadManualOverrides() {
    // v1.5.45: manual commits are physical schedule_json entries.
  ...
  function render(row, rows, sourceInfo) {
    ...
    $('scheduleLink').href = `${CONFIG.page}?month=${encodeURIComponent(month)}&v=1.5.45`;
    $('homeLink').href = 'index.html?v=1.5.45';
