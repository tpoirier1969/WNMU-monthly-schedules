(function () {
  ...
  const VERSION = 'v1.5.85-cell-menu-custom-tag-save-stabilizer';
  ...
  function writeProgramPlanningTags(target, tags, otherLabel) {
    ...
    const cleanTags = normalizeTagsForStorage(rawTags);
    const cleanOther = cleanOtherLabel(otherLabel || '');
    if (cleanOther) cleanTags[OTHER_KEY] = true;
    else delete cleanTags[OTHER_KEY];
    ...
    if (entryId) {
      ...
      if (!Object.keys(cleanTags).length && !cleanOther && !next.rectNote && !next.text && !next.cellOverrideV15) delete all[entryId];
